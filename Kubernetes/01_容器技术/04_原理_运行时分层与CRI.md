...
## 1. 定义接口、实现接口与托管进程是三件事

容器最终是内核中一个被切分过的进程。如果让节点代理（`kubelet`）自己完成全部工作——自己拼接运行参数、自己调用底层工具，并且自己充当容器进程的父进程——三类问题会同时出现：
...
划分方式因此变成三段：**定义接口**、**实现接口**、**托管进程**。前两者由一个标准接口分开，第三者则交给一个每容器（或每沙箱）各一个的轻量进程。
