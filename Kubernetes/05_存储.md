...
> 本笔记对应 [[Kubernetes/00_简介]] 的「阶段 5」。目标：能讲清「临时存储 / 持久存储 / 共享存储」三类需求分别由哪些卷类型承接，PV 与 PVC 如何把「谁提供存储」和「谁使用存储」解耦，动态供给与 CSI 由哪些组件分工完成，以及一个有状态应用从申请存储到回收数据的完整闭环（申请 → 绑定 → 挂载 → 扩容 → 快照 → 回收）。
>
