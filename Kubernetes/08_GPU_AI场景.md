...
> 本笔记对应 [[00_简介]] 的「阶段 8」。目标：能讲清 GPU 是**怎么变成** Kubernetes 可调度资源的、NVIDIA GPU Operator 到底装了哪些组件、三种 GPU 共享方式（时间切片 / MIG / MPS）的取舍、多卡训练为什么同时需要**拓扑感知**与 **gang 调度**、训练网络（RDMA/RoCE）怎么接进来，以及 GPU 监控告警怎么落地。
>
