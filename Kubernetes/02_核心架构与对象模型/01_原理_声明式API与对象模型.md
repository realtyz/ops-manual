...
## 1. 问题背景：把"怎么一步步做"交给调用方

假设运维动作只有一种表达方式：调用方发送一串指令，每一步都必须成功，且顺序必须正确。此时会同时出现三类问题：
...
需要提前记住的一条边界是：`resourceVersion` 是**不透明**的。它当前使用一个单调递增的数字，但这一性质并不属于它的契约；任何"用版本号排序或判断新旧"的做法都不可靠，正确用法只有等值比较（`resourceVersion=`）以及"不小于"（`resourceVersionMatch` 之类的语义由服务端定义）。
...
4. **版本号不是新旧标尺。** 它是并发控制使用的令牌，只用于等值比较。
...
| 「声明式就一定会自愈」 | 自愈的前提是有控制器在管理这个对象；没有控制器的对象只能靠人 |
