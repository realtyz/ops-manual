...
| 字段区 | 谁写 | 含义 | 判据 |
| --- | --- | --- | --- |
| `spec`（少数对象是 `data` 等） | 调用方 / 控制器 | 期望状态：要什么 | **带 `status` 子资源的对象**才在 `spec` 变更时使 `metadata.generation` 递增 |
| `status` | 系统组件 / 控制器 | 观测状态：现在是什么 | 用户提交的 `status` 会被忽略 |

需要自己确认某个类型是否在期望态变更时递增代数时，可以用两次同口径读数对照。例如 `# 验证：ConfigMap 连改两次 data，看代数是否递增`：`kubectl create configmap gen-demo --from-literal=k=1`、`kubectl patch cm gen-demo --type=merge -p '{"data":{"k":"2"}}'`，再过 `kubectl get cm gen-demo -o jsonpath='{.metadata.generation}{"\n"}'`。`ConfigMap` 没有状态子资源，因此它的该字段通常停在同一值——这说明"改期望态就递增代数"不是对象模型的通用性质，而是带状态子资源那类对象的性质。
...
`409` 的正确处理是**重读、重算、重提**，而不是重试同一个请求体。需要复现这个保护机制时，可以先取一个版本再故意用它提交：`# 验证：用过期版本提交会得到 409`，即 `RV=$(kubectl get cm gen-demo -o jsonpath='{.metadata.resourceVersion}')` 之后再以该版本提交一次已经落后于集群的修改。这也是"用脚本批量修改对象"经常失败的原因：脚本拿到的是旧版本，改到一半时别人已经修改过。
