...
> 本笔记对应 [[00_学习大纲|Linux 大纲]] 的「阶段 1：实验环境与操作纪律」。目标：**先有一台能快照、能重建、能进控制台的实验机**，把版本基线固化下来；知道主流发行版的差别在哪、同一条命令为什么换个家族就不成立；并且形成「先取证、再动手、留回滚」的习惯——后面 11 个阶段的每一篇笔记都建立在它上面。
>
> 关联复习：[[Linux/02_启动流程与内核/00_导读与知识地图|02 启动流程与内核]]（救援入口、GRUB 与 emergency 的处置在这里第一次用到）、[[Linux/03_进程与信号/00_导读与知识地图|03 进程与信号]]（PID 1 的信号语义、容器里的 PID 1）、[[Linux/05_存储与文件系统/00_导读与知识地图|05 存储与文件系统]]（打满磁盘与 fstab 写错的深水区）、[[Linux/06_网络协议栈与排障/00_导读与知识地图|06 网络协议栈与排障]]（`ip`/`ss`/`nstat` 这条工具链的来源）。
>
...
- **四项基线是「跨机器可比较」的最小集合**：同一个 `vm.swappiness` 在 RHEL 9 与 Ubuntu 24.04 上的默认值可能不同，不写清基线，笔记里的数字半年后就没法复用（这也是 [[Linux/04_内存管理与OOM/00_导读与知识地图|04 内存管理与 OOM]] 与 [[Linux/05_存储与文件系统/00_导读与知识地图|05 存储与文件系统]] 反复强调「默认值一律查本机」的原因）。
- **`/proc/cmdline` 与运行时参数是两套东西**：前者只在启动时生效，后者用 `sysctl` 管；改法完全不同（详见 [[Linux/02_启动流程与内核/00_导读与知识地图|02 启动流程与内核]]）。
...
1. **内核版本号不能直接比大小**：RHEL 9 的基线内核是 **5.14**、Ubuntu 22.04 是 **5.15**、Ubuntu 24.04 是 **6.8**、Debian 12 是 **6.1**。但 RHEL 的做法是**把新特性回移（backport）进老内核**——所以「RHEL 内核版本低 = 功能少」是错的判断。要确认某个特性在不在，只能查该发行版的 release notes，或直接在本机试（这也是 [[Linux/04_内存管理与OOM/00_导读与知识地图|04 内存管理与 OOM]]、[[Linux/05_存储与文件系统/00_导读与知识地图|05 存储与文件系统]]、[[Linux/06_网络协议栈与排障/00_导读与知识地图|06 网络协议栈与排障]] 三篇都要求「实测 + 标注环境」的根本原因）。
...
- **man 页与发行版文档冲突时，以发行版文档 + 本机实测为准**：man 页来自软件上游，发行版会改默认值（`tuned`、`sysctl.d`、编译开关），[[Linux/04_内存管理与OOM/00_导读与知识地图|04 内存管理与 OOM]] 与 [[Linux/06_网络协议栈与排障/00_导读与知识地图|06 网络协议栈与排障]] 里的参数就属于这一类。
...
| 内存里的 core/`vmcore` | 取决于是否配置 | panic 场景见 [[Linux/02_启动流程与内核/00_导读与知识地图|02 启动流程与内核]] 的 kdump |
...
| **写坏 `/etc/fstab`** | 把某个 UUID 改错一个字符，或去掉 `nofail` 后写一个不存在的设备 | 重启时进 `emergency mode`，`systemctl --failed` 有失败挂载 | `emergency.target` 里 `mount -o remount,rw /` → 修 fstab → `mount -a` 验证 → `systemctl default`（详见 [[Linux/02_启动流程与内核/00_导读与知识地图|02 启动流程与内核]]） |
...
- **容器里的 PID 1 是「子命名空间的 init」，同样受这套规则保护；但从宿主机看它只是一个普通进程**：所以「容器里的 1 号进程杀不掉」和「宿主机上杀掉容器的 1 号进程」是两件事——后者用宿主机看到的 PID 就能杀（见 [[Linux/03_进程与信号/00_导读与知识地图|03 进程与信号]] 的「退出与回收」与「隔离：namespace 与 cgroup」两篇）。这也是为什么有些容器停不掉要靠 `docker rm -f` 或宿主侧 `kill`。
- **PID 1 还负责回收孤儿进程**：容器里如果让业务进程直接当 PID 1，它通常不会 `wait()` 那些被它「继承」的孤儿，于是出现僵尸堆积——这是「容器里该不该用 `tini`/`--init`」这类问题的根源（同样见 [[Linux/03_进程与信号/00_导读与知识地图|03 进程与信号]]）。
...
| 重启后进 `emergency mode`（`fstab` 写坏） | 控制台 + `emergency.target` | `mount -o remount,rw /` → 注释/修正出错行 → `mount -a` 验证 → `systemctl default`（详见 [[Linux/02_启动流程与内核/00_导读与知识地图|02 启动流程与内核]]） |
| 登录报错、`su`/`sudo` 失败（`passwd` 改坏） | rescue/单用户，或从快照启动 | `vipw` 改回、`pwck` 校验；`/etc/shadow` 同理（`vipw -s`） |
| 系统起不来 / 文件系统坏了 | Live CD/USB 或云主机挂载恢复 | 挂载根分区到 `/mnt` 后改文件；文件系统的深水区见 [[Linux/05_存储与文件系统/00_导读与知识地图|05 存储与文件系统]] |
...
预期：写入在 64MB 附近失败并报 `No space left on device`；删除文件后空间立即恢复。**再进阶一步**：用 `df -i` 对比 inode 视角、用 `lsof +L1` 观察「删了文件但空间没释放」的现象（完整原理见 [[Linux/05_存储与文件系统/00_导读与知识地图|05 存储与文件系统]]）。
...
环境：**虚拟机/云主机 + 控制台**；改动前后都要确认「快照已打」。和 [[Linux/02_启动流程与内核/00_导读与知识地图|02 启动流程与内核]] 的 `emergency.target` 章节对照看。
...
预期：`echo c` 之后内核立即 panic 并重启（或停在 kdump 抓取阶段）；重启后 `journalctl -b -1` 能看到 crash 相关记录。**这是理解「panic 现场靠什么保留下来」最直接的方式**，也是 [[Linux/02_启动流程与内核/00_导读与知识地图|02 启动流程与内核]] 里 kdump 那一节的前置体验。
...
- [x] 跨笔记引用都用 wikilink；跨目录引用用路径写法（[[00_学习大纲|Linux 大纲]]）；指向尚未创建的阶段笔记（[[10_性能分析与排障]]）的链接与 [[00_学习大纲|00_学习大纲]] 的规划保持一致，目前在 Obsidian 中显示为未解析属预期
- [x] [[00_学习大纲|00_学习大纲]] 的阶段 1 已有「对应笔记」链接，且 checklist 已补上「发行版对比」这一项
