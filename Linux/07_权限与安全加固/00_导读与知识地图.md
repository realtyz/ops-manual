...
>
> 本篇与子笔记的默认值、字段与行为以上述资料为准。凡涉及发行版或内核差异，正文会标注适用环境并给出「查本机」的命令。**实测环境：Ubuntu 24.04.5 LTS（VMware 虚拟机）/ 内核 6.8.0-139-generic / systemd 255（255.4-1ubuntu8.17）/ cgroup v2 / root。** 本章的账号文件权限、DAC 与 ACL、`chattr`、capabilities、`sudo`/`su`、真实 `sshd` 服务端、AppArmor、auditd、挂载选项与关键 sysctl 均已在本机真跑并回填；**仍未实测**的是 SELinux（本机未安装，需 RHEL 9 或改引导参数）与 `rpm -Va`（本机无 RHEL）、以及需要控制台或快照才能做的自锁类演练（改 PAM/`sshd_config` 到锁死状态、写坏 `/etc/fstab`）。未实测的部分在子笔记中逐条标注，换环境必须重采。
...
> [!info] 读之前：先自检三条前提
> 1. **有一台能 root 的实验机**（虚拟机最佳、可快照）。本笔记的实测环境就是一台可 root 的 VMware 虚拟机，所以 `sudoers`、`chattr`、capabilities、真实 `sshd` 服务端、AppArmor、auditd 这些需要 root 的篇目都做了实测；但**凡可能把自己锁在外面的变更（改 `/etc/pam.d/`、把 `sshd_config` 改到锁死登录、写坏 `/etc/fstab`）仍然只在副本上校验语法，不在生产机上第一次动手**。
...
> - **SUID 只对二进制生效，脚本上的 SUID 被内核忽略**：`execve()` 在 `no_new_privs`、`nosuid` 挂载、被 `ptrace` 时也会忽略 SUID/file capabilities。实测本机 SUID 文件 **13** 个、SGID 文件 **7** 个、带 file capabilities 的文件 **4** 个；核查要用 `find -perm -4000`（包含），不是 `-perm 4000`（精确）。注意 `chmod 4755` 之后再加 `chown` 会**把 SUID 位清掉**，这是「明明设了 SUID 却不生效」的常见原因。
...
> - **账号默认值几乎都是「不过期」，但「root 密码被锁」这句在本机不成立**：`PASS_MAX_DAYS=99999` 来自 `/etc/login.defs`（实测），而 `INACTIVE=-1` 其实**不是 `login.defs` 里的项**——本机 `login.defs` 没有 `INACTIVE`，`-1` 只来自 `useradd -D` 的输出（实测）。**本机 root 的 shadow 密码字段首字符是 `$`（已设置哈希），不是 `!*`**；`!*` 属于另外 **5** 个账号（实测：2 个 `HASHED`、28 个 `LOCKED/NO-LOGIN-PW`、5 个 `OTHER(!*)`）。加固必须主动收紧有效期、失败锁定和 root 登录入口。
> - **`sudo` 与 SSH 是最常被走的合法通道，也是最该留痕的地方**：sudo 日志含 `TTY=`、`PWD=`、`USER=`、`COMMAND=`；SSH 侧用 `sshd -T -C user=...,addr=...` 看 `Match` 后的最终生效值，不要只看配置文件写没写。
> - **加固的验收标准是「可核查」，不是「感觉关掉了」**：定期查五类基线——账号、SUID/SGID/capabilities、监听端口、计划任务与自启、SSH 授权；`setenforce 0` 只把强制拦截关成 permissive（**审计与 AVC 记录仍在**），不是修复。
