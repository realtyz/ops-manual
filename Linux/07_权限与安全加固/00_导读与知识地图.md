...
**「提权面」指所有能把普通身份变成更强身份的路径的集合**，不止 SUID 位：还有 file capabilities、`sudo`/`su` 的授权、以 root 执行但可被非 root 写入的脚本与配置、SSH 钥匙、动态链接器、内核与设备接口、容器特权。**它们中的大多数平时完全静默**，所以核查靠的是与留档清单比对，而不是当下的印象。
