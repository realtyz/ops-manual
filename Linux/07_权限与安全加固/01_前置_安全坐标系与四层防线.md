...
>
> 本篇结论来自上述资料；具体命令输出如无特别说明，以本机实测为准。凡涉及 SELinux/AppArmor 的输出，Ubuntu 24.04 WSL2 环境不支持时已标注「未实测」。
...
> - **`EACCES`（Permission denied）通常是 DAC 或路径问题**；`EPERM`（Operation not permitted）常是 capability、挂载选项、不可变属性或 MAC 拒绝。
> - **文件 777 不保证能访问**：路径上缺 `x`、ACL mask、`nosuid`/`noexec`、capability 不足、SELinux/AppArmor 拒绝，任何一层都能拦下。
...
| MAC | 即使 DAC 同意，全局策略允不允许 | `avc: denied`、AppArmor `DENIED` | `ausearch -m AVC`、`aa-status`、`journalctl -k` |
...
5. **SELinux/AppArmor 开着吗？** 开着 → 查 AVC/AppArmor 日志，看是不是它拦的。
...
> - `EACCES`：优先看路径逐级 `x`、文件 mode、ACL 有效权限。
> - `EPERM`：优先看 capability、`chattr +i`、只读挂载、MAC 拒绝。
