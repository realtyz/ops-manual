...
>
> **实测状态**：本篇不新增实测；引用的数字**转引自前十一章在本实验机的实测记录**（转引条件以对应阶段为准），其中的 cgroup 实验在本实验机用**系统级** `systemd-run`（root）完成。本实验机为 **Ubuntu 24.04.5 LTS（VMware 虚拟机）/ 内核 `6.8.0-139-generic` / systemd 255 / 4 vCPU / `MemTotal` 7894 MiB / root 可用**。**未实测**：真实 Kubernetes 集群、CNI/CSI 插件、云盘在线扩容、云上 CPU 积分——本实验机不具备这些环境，相关段落只给「宿主侧该看什么」与验收标准。
...
>   - 证据：`systemd-detect-virt` 报虚拟化类型（本实验机报 `vmware`）；DMI 由虚拟化平台提供，**本实验机是齐全的**（`sys_vendor=VMware, Inc.`、`product_name=VMware Virtual Platform`，`dmidecode`/`lspci`/`lshw` 都可用），有的平台会缺失，所以「DMI 为空」不能当通用判据；云盘扩容要「卷 → 分区 → 文件系统」三跳。
...
- 实测参照（**转引**阶段 10）：`CPUQuota=20%` 对应 `cpu.max=20000 100000`，`cpu.stat` 报 `nr_periods 21 / nr_throttled 20`，同一段代码的迭代数从 29823329 掉到 5757053——**吞吐只剩 19.30%**。
...
| 层 | 与物理机的差异 | 怎么确认 | 影响什么 |
| --- | --- | --- | --- |
| **虚拟化层** | 硬件信息由平台提供，可能伪造或缺失；时钟可能受宿主影响 | `systemd-detect-virt`（本实验机报 `vmware`）；`ls /sys/class/dmi/id/`（**本实验机齐全**：`sys_vendor=VMware, Inc.`、`product_name=VMware Virtual Platform`、`bios_version=6.00`） | 固件、`smartctl`、NUMA 信息不一定可靠；但 DMI 是否为空要实测，不能假设 |
...
> [!example]- 生产动作：给三个容器现象各写一条宿主侧取证路径（纸面，或本机用系统级 `systemd-run` 模拟）
> **怎么做**：选 OOM、CPU 限流、网络不通三个现象，各写「宿主侧看什么文件/命令 → 期望形态是什么 → 判定是什么」。能本机模拟的（OOM、限流）用 `systemd-run --scope -p MemoryMax=` / `CPUQuota=` 做一遍（本实验机 root 可用，直接用**系统级** scope，不再需要 `--user`）。
> **预期**：你会发现自己原来只会从容器里看，遗漏了宿主侧一半证据。
> **风险**：低（系统级 scope + 明确上限；跑完确认 scope 已结束、无残留进程）。**耗时**：约 40 分钟。**回滚**：结束 scope、清理自建目录。
...
| 把缺硬件信息当成系统故障 | 部分虚拟化平台不提供 DMI；本实验机 `dmidecode`/`lspci`/`lshw` 都可用，「虚拟机上 DMI 一定为空」不是通用判据 |
