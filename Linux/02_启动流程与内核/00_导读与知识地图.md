...
| #   | 站点            | 谁在跑                              | 它凭什么交棒                                              | 交不出去时的落点                                                 | 到这里要能说清                                                  |
| --- | ------------- | -------------------------------- | --------------------------------------------------- | -------------------------------------------------------- | -------------------------------------------------------- |
...
**贯穿全章的一个三问**：控制权现在在谁手里；它要靠什么才能把棒交出去；交不出去时现场在哪个屏幕或哪份日志里。

### 三条横切线

| 横切线 | 贯穿内容 | 收口于 |
| --- | --- | --- |
| 存储标识线 | 分区/卷名 → PARTUUID → 文件系统 UUID → `root=` → `fstab` 来源 | 「分区迁移或克隆之后起不来」怎么一次核完三处 |
| 参数与模块线 | 内核命令行 → initramfs 内容 → 模块参数 → `sysctl` → unit 文件 | 「配置写了却不生效」先问它什么时候被读 |
| 证据线 | 固件屏/带外日志 → initramfs shell → 内核环缓冲 → journal → `vmcore` → pstore | 「先取证还是先止损」按证据所在介质决定 |

## 术语速查

| 名词 | 一句话解释 | 在哪一篇展开 |
| --- | --- | --- |
| 固件 / POST | 主板上的第一段程序 / 它的加电自检 | 原理：启动链为什么要分阶段接力 |
| UEFI / legacy BIOS | 两代固件；差别在"从哪里加载引导器" | 原理：启动链为什么要分阶段接力 · 体系 |
| ESP | EFI System Partition，UEFI 存放引导器的小分区（FAT） | 原理：启动链为什么要分阶段接力 · 体系 |
| 启动项 | UEFI 存在 NVRAM 里的"设备路径 + `.efi` 文件路径"记录 | 原理：启动链为什么要分阶段接力 |
...
| `grub.cfg` / BLS 条目 | 生成的菜单配置 / RHEL 8/9 默认的"一个内核一个小文件"约定 | 原理：启动链为什么要分阶段接力 · 体系 |
| `grub>` / `grub rescue>` | GRUB 的正常模式 / 精简模式（模块未加载） | 实操 · 判例 |
| `vmlinuz` | 压缩过的内核镜像 | 原理：启动链为什么要分阶段接力 |
| initramfs / initrd | cpio 归档形式的临时根 / 旧的块设备镜像形式 | 原理：启动链为什么要分阶段接力 |
| `root=` | 指定根文件系统所在设备的命令行参数 | 原理：从内核到 PID 1 与用户态 |
| `rd.*` | 归 initramfs 用的一组参数前缀（与 `systemd.*` 是两套） | 原理：从内核到 PID 1 与用户态 |
| `/sysroot` / `switch_root` | 停在 initramfs 时真根的挂载点 / 把 `/` 换成真根并启动 PID 1 | 原理：启动链为什么要分阶段接力 |
...
| panic / oops | 内核判定无法继续 / 内核出了错但尽量继续跑 | 原理：崩溃现场为什么留不住 |
| `kernel.panic` | panic 后等多少秒重启；`0` 表示停在现场 | 原理：崩溃现场为什么留不住 |
| `kernel.panic_on_oops` | 把 oops 升级成 panic 的开关 | 原理：崩溃现场为什么留不住 · 治理 |
| `vm.panic_on_oom` | 决定 OOM 是否升级为 panic（默认不） | 原理：崩溃现场为什么留不住 |
| kdump / `crashkernel=` | 用捕获内核保存崩溃现场 / 为它预留内存的参数 | 原理：崩溃现场为什么留不住 |
| `vmcore` / `vmcore-dmesg.txt` | 崩溃内存镜像 / 从中抽出的纯文本崩溃前日志（无需调试信息） | 原理：崩溃现场为什么留不住 |
| kexec | 不经过固件直接换内核的机制，kdump 也用它 | 治理 |
| pstore / ramoops | 写到重启不清空区域的日志后端 | 原理：崩溃现场为什么留不住 |
...
| 改了内核参数却不生效 | 横切：参数 | `/proc/cmdline`、`sysctl`、配置文件名与编号 | 不要只盯 `/etc/default/grub` 重改一遍 |
| 驱动加载失败、设备不认 | 横切：模块 | `journalctl -k -b`、`modinfo`、DKMS 状态 | 不要用 `lsmod` 没有就断定内核不支持 |
...
| # | 篇目 | 一句话 |
| --- | --- | --- |
| 01 | [[Linux/02_启动流程与内核/01_原理_启动链为什么要分阶段接力\|原理：启动链为什么要分阶段接力]] | 四段接力各自解决什么；initramfs 要解的死锁 |
...
| 04 | [[Linux/02_启动流程与内核/04_原理_崩溃现场为什么留不住\|原理：崩溃现场为什么留不住]] | 三级错误严重程度；让现场活下来的四类载体 |
| 05 | [[Linux/02_启动流程与内核/05_体系_一次启动的完整交接\|体系：一次启动的完整交接]] | 五段交接一条线走完，含三条横切线与字段读法 |
...
| 09 | [[Linux/02_启动流程与内核/09_验收_盲区排查\|验收：盲区排查]] | 八类现象逐层剖析、三条横切线的四层定位、缺口类型与补法、交付物与核对方式 |
