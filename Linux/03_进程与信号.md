...
>
> 说明：结论以 man 页（`proc(5)`、`signal(7)`、`cgroups(7)`、`systemd.exec(5)`）与内核源码为准；凡涉及默认值、内核差异与发行版差异的地方都标注适用环境（本文以 **RHEL 8/9 系** 与 **Debian/Ubuntu LTS 系** 为主线，cgroup 以 **v2** 为准，v1 差异单独标注）。文中的「预期输出」目前都是文档值而非实测值，标记约定见文末的「验证进度」。

> [!info] 读之前：这篇笔记假设你已经具备三件事
> 1. **一台可快照的实验机**（虚拟机即可）**并且有 root**。第 4 节里有四个实验要动块设备、cgroup 限制和 systemd 单元，破坏性动作只在它上面做。
> 2. **会直接读 `/proc`**：知道 `/proc/<pid>/status`、`/proc/<pid>/limits` 这类文件是内核暴露的实时状态。阶段 2 的 [[02_启动流程与内核]] 已经带你用过 `journalctl`、`lsblk`，从那里接着走即可。
> 3. **基本的进程观测与 systemd 操作经验**：`ps`、`top`、`systemctl status/restart`。不熟可以先看 [[01_实验环境与操作纪律]]。
>
> 另外两个环境前提：实验 5 需要 `device-mapper`（`dmsetup`），实验 6 需要 **cgroup v2**（先跑 `stat -fc %T /sys/fs/cgroup`，输出 `cgroup2fs` 才算满足）。环境不满足时正文照样能读，只是实验做不全。
...
> [!tip] 怎么用这篇笔记：分三遍读
> 全篇按难度分了两层：标 **【核心】** 的属于阶段 3 必须掌握（尤其是「卡死、杀不掉、起不来、悄悄消失」这四类问题）；标 **【进阶】** 的只需要知道有这条路，用到时再回来查。
> - **第一遍（约半天，先建立直觉）**：第 0 节 → 1.1 进程与线程 → 1.2 进程状态 → 1.3 信号 → 1.4 三种死法 → 第 4 节的实验 1、2。这两个实验成本最低、收益最高，而且都不会破坏系统。
> - **第二遍（约一天，把资源和排障接上）**：1.5 core dump → 1.7 单进程的限制 → 1.8 五层结构 → 1.9 资源耗尽 → 1.10 僵尸与孤儿 → 1.11 namespace → 1.12 cgroup → 2.2 优雅退出 → 3.1–3.5 → 实验 3、4。
> - **第三遍（进阶与破坏性演练）**：1.6 优先级与调度 → 2.3 资源限制治理 → 2.4 常见坑 → 3.6、3.7 → 实验 5、6。挂起块设备与触发 cgroup OOM/fork 失败会真的把机器弄进异常状态，放在最后练，动手前先打快照。
...
> | 名词 | 一句话解释 | 在哪一节展开 |
> | --- | --- | --- |
> | task / TGID / TID | 内核的调度单位 / 线程组 ID（用户态看到的 PID）/ 线程 ID | 1.1 |
> | `LWP` | `ps -eLf` 里的线程 ID 列 | 1.1 |
> | `R/S/D/T/t/Z/I` | `ps` 的进程状态字母，`D` 是内核态不可中断等待 | 1.2 |
> | `wchan` / `stack` | 进程正睡在哪个内核函数上 / 内核态调用栈 | 1.2、3.1 |
> | `TASK_KILLABLE` | 不可中断睡眠的一个变种，但能被致命信号打断，仍显示为 `D` | 1.2 |
> | `128+N` | shell 里「被信号 N 杀死」的退出码表示法（143 = TERM、137 = KILL） | 1.4 |
> | OOM Killer | 内存不足时由内核挑选进程并 `SIGKILL` 的机制 | 1.4、3.3 |
> | `core_pattern` | 决定 core 写到哪里、或通过管道交给谁的内核参数 | 1.5 |
> | `coredumpctl` | systemd 系统上查看与调试 core 的工具 | 1.5 |
> | nice / `chrt` / `taskset` | CPU 权重 / 调度策略 / CPU 亲和性，三者管的事不同 | 1.6 |
> | `RLIMIT_*` / 软限制 / 硬限制 | `setrlimit` 定义的限制项 / 当前生效值 / 软限制的上限 | 1.7 |
> | `prlimit` | 查看并在线调整运行中进程 limit 的工具 | 1.7 |
> | PAM | 登录认证框架；`limits.conf` 只影响经它建立的会话 | 1.8 |
> | `LimitXXX=` / `TasksMax=` | systemd unit 里的资源限制项 | 1.8 |
> | `EAGAIN` / `ENOMEM` | 「资源暂时不可用」/「内存不足」，两类 fork 失败的报错 | 1.9、3.4 |
> | subreaper | 孤儿进程被重新挂靠的目标，通常是 PID 1 | 1.10 |
> | namespace | 隔离「看得见什么」（`mnt`/`pid`/`net`/`user`…） | 1.11 |
> | cgroup v2 / `subtree_control` | 单一层级的资源控制树 / 向子树委派控制器的方式 | 1.12 |
> | `pids.max` / `memory.max` | cgroup 的任务数上限 / 内存上限 | 1.12 |
> | capabilities / seccomp | 权限约束的两层机制，与 namespace、cgroup 一起构成容器 | 1.12 |

### 1.1 进程与线程：内核眼里它们是同一类东西【核心】
...
> [!check] 读完 1.2 应该能回答
> 为什么「D 状态一定杀不掉」只说对了一半？看到 D，你接下来该查哪两样东西？——下一节讲进程之间怎么互相通知。

### 1.3 信号：进程之间唯一被内核承认的「异步通知」【核心】
...
3. **PID 1 受内核特殊保护**：内核对 PID 1 **不会执行信号的默认动作**——凡是它没有显式安装处理函数的信号，一律被忽略。这条规则在容器里会变成事故（见 1.10）。
...
> [!check] 读完 1.4 应该能回答
> 看到退出码 137，能不能直接断定是 OOM？要拿到哪条证据才能定性？——下一节是它的对照组：进程不是「退出」而是「崩溃」时，现场在哪。

### 1.5 用户态崩溃取证：core dump【核心】
...
### 1.6 优先级与调度：nice、chrt、taskset 各管什么【进阶】
...
> [!check] 读完 1.6 应该能回答（进阶题）
> nice 与 cgroup 的 `cpu.weight` 分别作用在哪一层？为什么把 nice 调到 19 的进程，在空闲机器上照样能跑满一个核？

### 1.7 单个进程的限制：`ulimit` 与 `setrlimit`【核心】

`setrlimit(2)` 定义的那组限制（`ulimit` 就是它的 shell 前端）**每个进程各持一份**：软限制是当前生效值，硬限制是软限制的上限；无特权时降低硬限制之后就再也提不回来，这是「手滑把服务锁死」的经典方式。
...
这张表不用背：**排障时先记 `-n`（fd）与 `-u`（进程数）两行，再用 `cat /proc/<pid>/limits` 看进程真正生效的值**，其余几项遇到报错再回来查。
...
> [!check] 读完 1.7 应该能回答
> 软限制与硬限制的区别是什么？为什么 `ulimit -n` 改完只对「当前 shell 及其子进程」有效？——不过真正让人栽跟头的不是「限制是多少」，而是「这条限制该改在哪一层」，下一节讲这个。

### 1.8 五层结构：改动写在哪一层【核心】

同一个「上限」，可能来自五个完全不同的层次；**改错地方就等于没改**。这是「我明明改了怎么不生效」的唯一正解，也是面试高频题：
...
cat /proc/<pid>/limits                # 验证：服务进程实际拿到的值（与上一行的配置值对比）
```

> [!check] 读完 1.8 应该能回答
> 服务报 `too many open files` 时，改 `/etc/security/limits.conf` 为什么没用？正确的改法是什么，改完用什么验证？——下一节把这五层「撞上去」的样子具体化。

### 1.9 资源耗尽：进程数、线程数与句柄【核心】

1.8 讲的是「限制配在哪一层」，这一节讲「撞上去时是什么样子」：进程数与句柄数各自有好几条独立的上限，报错文案相似但判据完全不同——先分清撞的是哪一条，再谈怎么改。
...
> [!check] 读完 1.10 应该能回答
> 为什么「杀僵尸」是无效动作？容器里 PID 1 是 `bash` 时，`docker stop` 为什么要等满 10 秒？——最后两节讲容器赖以成立的另外两块机制。

### 1.11 namespace：隔离「看得见什么」【核心】

这是本阶段最后一块，也是通往容器的桥。一句话区分：**namespace 改的是可见性，cgroup 改的是资源上限**；两者互不替代。这一节先讲 namespace。
...
> [!check] 读完 1.11 应该能回答
> namespace 解决的是「看得见什么」还是「能用多少」？容器内 PID 1 的编号是从哪来的？——最后一节讲限制那一半。

### 1.12 cgroup：限制「能用多少」【核心】

cgroup v2 的控制器是一组可读写的文件（**v2 是单一层级**，靠 `cgroup.subtree_control` 向子树委派控制器）。下面这张表**第一遍只需要记住 `memory`、`pids`、`io` 三行**，其余用到再查：
...
> [!check] 读完 1.12 应该能回答
> namespace 与 cgroup 各管什么、为什么不能互相替代？构成「容器」的四层是哪四层？——第 1 节的机制讲完了，下一节把它们落到生产上。
...
> **最低成本复现**：第 4 节实验 5（用 `dmsetup suspend` 挂起一个环设备，再对它做写操作）。环境不允许动 device-mapper 时，可以退而观察 `T` 状态（`kill -STOP` 后进程可被 `SIGCONT` 唤醒）与 `D` 的差别，或用一台不可达的 NFS `hard` 挂载做对照。
...
> **最低成本复现**：非交互 shell 里跑 `bash -c 'sleep 0.1 & sleep 60'`，在第二个 `sleep` 期间用上面的命令就能看到一条 `Z`；等父进程退出、子进程被 PID 1 回收后，它就消失了。
...
> **最低成本复现**：第 4 节实验 2 覆盖「被信号杀」（人为 `kill -9` 一个测试进程，观察 137），实验 6 覆盖「被 OOM 杀」（cgroup 级 OOM 后 `memory.events` 里的 `oom_kill` 计数增加）。两种死法的证据位置正好可以对照着看。
...
> **最低成本复现**：第 4 节实验 6 的 `TasksMax=5`——在 `systemd-run` 起的临时单元里故意 fork 20 个后台进程，内核日志里就会出现 `cgroup: fork rejected by pids controller`。
...
> **最低成本复现**：在实验机上用 `bash -c 'ulimit -n 16; exec <一个要开很多文件或连接的命令>'` 就能撞出报错，再用上面的 `ls -l /proc/<pid>/fd` 看清是套接字还是普通文件占满的。第 4 节实验 3 还会证明「为什么 `limits.conf` 改不动它」。
...
> **最低成本复现**：用 `bash -c 'while :; do :; done'` 起一个单线程死循环占满一颗核，再用 `top -H` 与 `mpstat -P ALL 1` 对照——进程级 CPU 接近 100%，整机却只有 1/N。
...
> **最低成本复现**：`docker run --rm -it <镜像> sh` 起一个 PID 1 是 `sh` 的容器，`docker stop` 会等到超时后强杀；再加 `--init`，或把 `CMD` 改成 `exec` 形式让应用成为 PID 1，对比两次的停机时间与退出码。

## 4. 动手验证

> [!warning] 前置纪律
> 六个实验**按风险从低到高排列，建议按顺序做**：实验 1、2 只影响你自己起的测试进程；实验 3、4 可回滚（临时改 `limits.d`、产生 core，注意磁盘）；实验 5、6 会真的把机器弄进异常状态（挂起块设备、触发 cgroup OOM 与 fork 失败），**只在可快照的实验机上做，动手前先打快照**。生产机上只做只读观测。
...
> 耗时：约 10 分钟。
> 回滚：脚本最后一行的 `kill %1` 即可；确认没有残留的 `sleep` 进程。
...
> 耗时：约 15 分钟。
> 回滚：两个测试进程都会被杀掉，无需清理；`/tmp/lab-sig.sh` 可以留着复用或删掉。

> [!example]- 实验 3：句柄上限的三层结构（顺便证明 `limits.conf` 管不到 systemd 服务）
> 本实验覆盖 1.8 五层里的**会话、systemd 服务、PAM 登录**三层；另外两层（cgroup、内核全局）在实验 6 与 2.1 的基线里。
> ```text
> # 第一层：当前会话
...
> 风险：中，会临时改 `/etc/security/limits.d/` 并重启一个服务——务必用测试服务，不要拿业务服务做。
> 耗时：约 30 分钟（含一次重新登录）。
> 回滚：删除 `99-lab.conf`、停止 `lab-limits` 单元（脚本最后两行已包含）。
...
> 环境：任意发行版。
> 风险：中，core 文件可能很大，**注意磁盘**；`ulimit -c unlimited` 只影响当前会话。
> 耗时：约 20 分钟。
> 回滚：把 `ulimit -c` 恢复为 0 或系统默认值；清理 `/var/lib/systemd/coredump/` 与 `coredumpctl --vacuum-time=1s`。

> [!example]- 实验 5：亲手造一个 `D` 状态，看 `kill -9` 为什么无效
> ```text
...
> 风险：高，会挂起一个块设备，期间对它的一切 IO 都会阻塞；清理不彻底会留下 loop 设备与挂载残留。
> 耗时：约 30 分钟。
...
> 风险：高，会主动触发 cgroup 级 OOM 与 fork 失败；用 `systemd-run` 起的临时单元不会波及其他服务，但仍建议只在实验机做。
> 耗时：约 30 分钟。
> 回滚：`systemctl stop` 两个临时单元即可；重启后会自然消失。

## 5. 要点自测

题目前带 **【进阶】** 的对应笔记里的进阶小节，答不上来不影响阶段 3 过关；其余题目要求能盖住答案脱稿讲 3~5 分钟。
...
> [!question]- 【进阶】nice、cgroup 的 `cpu.weight`、`taskset` 分别解决什么问题？
...
- [ ] 【进阶】能讲清 nice、`chrt`、`taskset` 与 cgroup 权重/配额的分工
- [ ] 能说清 `ulimit`、`limits.conf`、systemd `LimitXXX=`、cgroup、内核全局**五层**的生效范围，并用 `/proc/<pid>/limits` 验证
...
本文的结论来自 man 页、内核源码与发行版文档，**尚未在实验机上逐条复现**；涉及默认值的地方写的是「查本机」的命令而不是断言数字，文中所有「预期输出」目前也都是文档值。标记约定：

- **✅ 已实测**：在真实实验机上跑过、输出与文中一致，并回填了实测环境（发行版 / 内核版本 / cgroup 版本 / systemd 版本）。
- **⚠️ 文档值**：来自 man 页或发行版文档、未实测；本机跑出来不一致时以本机输出为准，并回来更新这一条。

当前进度：**全篇为 ⚠️**。建议先取一份版本基线（`systemd --version`、`stat -fc %T /sys/fs/cgroup`、`ulimit -Sa`、`sysctl kernel.pid_max`），再从第 4 节的实验 1 开始逐个做，把实测输出与踩坑回填到对应小节，然后按小节把这里改成 ✅。
...
- [x] 每个排障小节都给了「最低成本复现」或明确的替代路径
- [x] 资源限制的层数在速览、1.8 与「回到路线图」三处已统一为**五层**
- [ ] 每条命令都附了「验证什么」与预期输出，并在真实实验机上跑过
- [ ] 第 4 节的六个实验（实验 1–6）全部实操完成，把实测输出与踩坑回填到对应小节
