...
> 本笔记对应 [[Linux/00_简介|Linux 大纲]] 的「阶段 2：启动流程、内核与系统初始化」。目标：把从加电到登录提示符的每一环讲成一条链，知道每一环失败时长什么样、现场在哪里，并且能在一台起不来的机器上按顺序把它救回来。
>
> 关联复习：[[08_systemd与服务管理]]（unit 依赖与服务的深水区）、[[10_性能分析与排障]]（开机慢属于它的方法论）、[[05_存储与文件系统]]（fstab 与根分区）。
>
> 说明：命令、默认值与行为以 man 页和发行版官方文档为准；凡涉及默认值、内核差异与发行版差异的地方都标注适用环境（本文以 **RHEL 8/9 系** 与 **Debian/Ubuntu LTS 系** 两个目标环境为主线）。文中的「预期输出」目前都是文档值而非实测值，标记约定见文末的「验证进度」。

> [!info] 读之前：这篇笔记假设你已经具备三件事
> 1. **一台可快照的实验机**（虚拟机即可）。破坏性实验只在它上面做；没有的话，第 3、4 节的动手部分只看不动。
> 2. **能进控制台**：虚拟机的串口或图形控制台，或者带外管理（IPMI/iDRAC/iLO）、云厂商的 VNC。启动流程的排障全部发生在登录之前，只靠 SSH 够不着现场。
> 3. **会最基本的 `mount`、`lsblk`、`journalctl`**：知道「挂载」是把一个文件系统接到目录树上、`/etc/fstab` 记录开机要挂哪些目录就够了。不熟可以先看 [[01_实验环境与操作纪律]]；挂载的深水区在阶段 5 的 [[05_存储与文件系统]]。
>
> 本文出现的生词（`ESP`、`switch_root`、`rd.break`、`kexec` 之类）在 1.1 末尾有一张「名词速查」表，读到卡壳就回来查。

## 0. 30 秒速览

> [!abstract] 这一页只要记住六句话
> - **启动链只有五环**：固件（BIOS/UEFI）→ GRUB2 → 内核 + `initramfs` → `systemd`（PID 1）→ `default.target`。前两环还归固件管，**从内核开始就是操作系统自己在给自己接生**。
> - **每一环失败了都有各自的「现场」**：固件看物理屏与带外日志，GRUB 看 `grub>` / `grub rescue>`，内核看 `dracut:/#`，systemd 看 `emergency.target` 与 `journalctl -xb`。判断故障的第一问是「现在是谁在输出」。
...
> - **先取证，再动手**：panic 与启动失败的第一手证据在 `journalctl -b -1`、`dmesg` 的引导段、`vmcore` 里，重启一次就没了；能进 GRUB 才能改参数进 `emergency.target` / `rd.break`，GRUB 都进不去就只能靠带外管理或 Live 介质。**所以救援通道要在故障之前就演练过，写进巡检，而不是等出事再找。**

> [!tip] 怎么用这篇笔记：分三遍读
> 全篇按难度分了两层：标 **【核心】** 的是阶段 2 必须掌握的；标 **【进阶】** 的只需要知道有这条路，用到时再回来查。建议这样安排：
> - **第一遍（约半天，先建立直觉）**：第 0 节 → 1.1 五环主干 → 1.2 → 1.3 → 第 4 节的「实验 0」。这一遍全是只读操作，不碰任何破坏性步骤。
> - **第二遍（约一天，把参数、模块和排障接上）**：1.4 启动参数 → 1.5 内核模块 → 1.6 全景回查 → 2.1 版本基线 → 2.2 的升级四步 → 3.1–3.5 → 实验 1。
> - **第三遍（在快照实验机上）**：1.7 `sysctl` 进阶 → 2.2 的 `kexec`/Livepatch → 2.3 kdump → 3.6、3.7 → 实验 2、3、4、5。`rd.break` 重置密码与主动触发 panic 是面试最容易露底的两个动作，放在最后练，动手前先打快照。
...
首读这张图只要看两件事：**箭头方向**（谁把控制权交给谁），以及**断点在哪**。每一环失败时，屏幕或日志会停在完全不同的地方——第 1、2 环停在固件和 GRUB 自己的提示符上，第 3 环停在 `dracut:/#`，第 4 环停在 `emergency.target`。所以「起不来」的第一问永远是：**现在是谁在输出？** 完整的失败现象对照放在 1.6，读完后面几节再看一遍会更清楚。

> [!info]- 名词速查（读到哪里卡住，就回这里查）
> | 名词 | 一句话解释 | 在哪一节展开 |
> | --- | --- | --- |
> | POST | 加电自检，主板固件开机后做的第一件事 | 1.1 |
> | BIOS / UEFI | 主板固件；UEFI 是较新的那套，通常配 GPT 分区表 | 1.1 |
> | ESP | EFI System Partition，UEFI 用来存放引导器文件的小分区 | 1.1 |
> | MBR | 传统 BIOS 时代的分区表与引导记录 | 1.1 |
> | `grubx64.efi` / `boot.img` | UEFI / BIOS 下被固件加载的那个 GRUB 文件 | 1.1 |
> | `grub.cfg` | GRUB 的配置，记录菜单项与内核命令行 | 1.4、3.1 |
> | `vmlinuz` | 压缩过的内核镜像文件 | 1.1 |
> | `initramfs` | 内存里的临时根文件系统，负责挂上真根 | 1.2 |
> | `switch_root` | initramfs 把控制权交给真根的动作 | 1.2 |
> | `/sysroot` | 停在 initramfs 阶段时，真根被挂载的位置 | 1.2、1.3 |
> | PID 1 / `systemd` | 内核拉起的第一个用户态进程，所有服务的祖先 | 1.3 |
> | `target` / `default.target` | systemd 的「启动目标」，相当于旧说法的运行级别 | 1.3 |
> | `emergency.target` / `rescue.target` | 两种救援模式，区别在文件系统挂了多少 | 1.3 |
> | `rd.break` | 内核命令行参数，让启动停在 initramfs 阶段 | 1.3、3.7 |
> | `sulogin` | 救援模式启动前要求输入 root 密码的程序 | 1.3 |
> | `/proc/cmdline` | 本次启动实际使用的内核命令行 | 1.4 |
> | `grubby` | RHEL 系改启动项的工具，不依赖 `grub.cfg` 的具体路径 | 1.4 |
> | `sysctl` | 读写运行时内核参数的接口，与启动参数是两套东西 | 1.7 |
> | `dmesg` / `journalctl -k` | 内核日志的两个入口 | 1.5 |
> | `builtin` / `.ko` | 驱动编进内核 / 编成可加载模块 | 1.5 |
> | DKMS | 内核升级后自动重编第三方模块的框架 | 2.2 |
> | `kexec` | 不经过固件直接换内核重启的机制，kdump 也用它 | 2.2、2.3 |
> | `crashkernel=` / `vmcore` | 为 kdump 预留的内存 / panic 时抓到的内存镜像 | 2.3 |
> | `crash` | 分析 `vmcore` 的工具，需要匹配版本的 debuginfo | 2.3 |
> | `nofail` / `x-systemd.device-timeout=` | fstab 挂载项参数，避免一个可选挂载拖死启动 | 2.4、3.4 |

> [!check] 读完 1.1 应该能回答
> 从加电到登录经过哪五环？如果一台机器黑屏停在 `grub>`，故障出在第几环、该看哪个屏幕？

### 1.2 `initramfs`：为什么必须有它，什么时候必须重建【核心】

内核本身只带最小驱动集，而根文件系统可能位于 LVM、RAID、LUKS 加密卷、iSCSI 或 NFS 之上——**要挂根，先得加载这些东西的驱动，但驱动又在根文件系统里**，这是一个先有鸡还是先有蛋的问题。`initramfs` 就是把解这个死锁所需的最小用户态与驱动打成一个 cpio 归档，由内核在内存里解成一个临时根，跑完 `/init` 后再 `switch_root`（把内存里的临时根换成真正的根，并交接控制权）到真正的根。

下面这张表是 RHEL 系与 Debian 系的操作对照。**第一遍只需要记两件事**：你现在用的是哪一套（`dracut` 还是 `initramfs-tools`），以及「重建当前内核」那条命令。其余四行用到再查。
...
- 内核升级后自动重建失败（例如 DKMS 编译失败、`/boot` 空间不足），此时 `initramfs` 与内核不匹配，机器起不来。DKMS 指的是「内核升级后自动把这些第三方模块按新内核重编一遍」的框架，NVIDIA 驱动、部分存储驱动都走它。
...
> [!check] 读完 1.2 应该能回答
> `initramfs` 解决的是什么死锁？换掉存储控制器之后，为什么光改 `fstab` 没用？——下一节接着讲：真根挂上之后，systemd 是怎么把系统带起来的。

### 1.3 `systemd` 接管之后：target 体系与四种救援入口【核心】
...
四种「进去修东西」的方式差别很大，生产上必须分清。前两种是 systemd 提供的 target，后两种是绕过 systemd 的路径；**第一遍只需要重点看前两行**：
...
> `rescue.target` 与 `emergency.target` 都要过 `sulogin` 验证（`sulogin` 是救援模式启动前要求输入 root 密码的程序；systemd 的紧急 shell 同样会要 root 密码）。若 root 已被锁定，会看到 `Cannot open access to console, the root account is locked`——这不是故障，而是设计：**它能防住的就是「拿到控制台就等于拿到 root」这件事**。
> 所以真正能无条件拿到 root 的是 `init=/bin/bash` 与 `rd.break` 这类绕过认证的路径，也正因如此它们在物理接触面前等于零防护。

> [!check] 读完 1.3 应该能回答
> `emergency.target` 与 `rescue.target` 的差别是什么？为什么说「卡住了就有 root shell」是过时的经验？——下一节离开启动链，讲引导器传给内核的那串参数。

### 1.4 内核命令行参数：启动时生效的那一套【核心】

内核命令行（kernel command line）是引导器在载入内核时一起传过去的一串参数，它**只在启动的那一瞬间被读取**，之后的整个运行期都不再变化。要确认本次启动到底带了什么，唯一的真相是：

```text
cat /proc/cmdline                   # 验证：本次启动实际使用的完整内核命令行
```

名字很像的另一套东西是运行时参数（`sysctl`）：它写进 `/proc/sys`、立即生效、重启即失效。两者对照与 `sysctl` 的用法放在 1.7，这一节先把启动参数讲完。

**临时改与持久改的区别，是这一节最常考的一点**：

| 改法 | 怎么做 | 作用范围 | 典型场景 |
| --- | --- | --- | --- |
| 临时改 | GRUB 菜单按 `e` 编辑内核行，`Ctrl-X` 引导 | 仅这一次启动 | 排障、进救援模式 |
| 持久改 | 改 `/etc/default/grub` 后重新生成配置 | 之后每次启动 | 变更配置 |

**持久化的写法**（RHEL 与 Debian 的路径差异必须记牢）：
...
常见参数按用途分三组。**第一遍只需要记住「日常必知」那三行**，其余两组知道存在、用到再查：

| 分组 | 参数 | 作用 | 坑 |
| --- | --- | --- | --- |
| 日常必知 | `ro` / `rw` | 根文件系统的初始挂载方式 | 改密码用的 `init=/bin/bash` 常需 `rw`，或进 shell 后 `mount -o remount,rw /` |
| 日常必知 | `quiet`、`rhgb` | 少打内核日志 / 图形化启动画面（RHEL 常用） | 排障时要**去掉它们**，否则现场信息被吞掉 |
| 日常必知 | `console=ttyS0,115200n8` | 把内核与 systemd 日志输出到串口 | 云主机与服务器带外管理依赖它；漏配就只剩一句「机器没起来」 |
| 救援与排障 | `systemd.unit=` | 指定本次启动的目标 unit | 写错 unit 名会掉回 `rescue.target` 而不是报错重试 |
| 救援与排障 | `init=` | 指定 PID 1 | `init=/bin/bash` 下没有 systemd，服务、日志、`systemctl` 全都不可用 |
| 救援与排障 | `systemd.log_level=debug`、`rd.debug` | 打印 initrd / systemd 的详细日志 | 只在排障时临时加，日志量很大 |
| 救援与排障 | `crashkernel=` | 为 kdump 预留捕获内核的内存（进阶，见 2.3） | 预留过小 → `vmcore` 抓不全；过大 → 正常运行业务少一块内存（用 `kdumpctl estimate` 估） |
| 谨慎使用 | `selinux=0`、`enforcing=0` | 关闭或降级 SELinux | **生产事故的常见错误动作**：它掩盖问题并让系统处于无标签状态，正确做法是看 `ausearch`/`dmesg` 排坑 |
| 谨慎使用 | `net.ifnames=0` | 回到 `eth0` 式网卡命名 | 改了会导致网络配置与既有脚本失配，「改回来」往往比重装还麻烦 |

> [!check] 读完 1.4 应该能回答
> 临时改与持久改分别在哪一层做？为什么「改了 `/etc/default/grub` 但没重新生成配置」等于没改？——下一节讲内核模块，也就是第 3 环里「加载驱动」那件事。

### 1.5 内核模块：驱动是怎么被装进去的【核心】
...
| `module verification failed` / `Key was rejected by service` | Secure Boot 下模块没签名或被拒 | 用发行版签名模块，或按流程注册 MOK（Machine Owner Key，Secure Boot 下由机器所有者注册的密钥），**不要直接关 Secure Boot** |
...
> [!check] 读完 1.5 应该能回答
> `lsmod` 里没有某个驱动，能不能断定内核不支持它？驱动加载失败时，证据在哪两个地方？——下一节把五环串起来回看一遍，然后进入进阶的运行时参数。

### 1.6 全景回查：每一环失败时长什么样

到这里五环的机制都讲完了，回到开头的全景图再看第二遍——这次重点不是「每环做什么」，而是**每一环失败时你会在哪里**。这张表建议当索引用，排障时先查它定位环节。
...
> [!check] 读完 1.6 应该能回答
> 随手给一个现象（比如 `VFS: Unable to mount root fs`、`You are in emergency mode`），你能说出它在第几环、现场在哪个屏幕或日志里吗？

### 1.7 运行时内核参数：`sysctl`【进阶】

> [!tip] 第一遍可以跳过
> 这一节讲的是 1.4 里提到的那「另一套参数」：它不走内核命令行，而是通过 `/proc/sys` 直接读写**正在运行的内核**。两套参数的名字可能很像（比如 `kernel.pid_max` 与 `pid_max=`），但机制、持久化方式和排障方向完全不同。

| 维度 | 启动参数（见 1.4） | 运行时参数（本节） |
| --- | --- | --- |
...
**三种改法**的作用域和持久性完全不同。第一遍只需要记住最后一行：
...
ls -l /etc/sysctl.d/99-sysctl.conf     # 验证：本机的老配置入口是不是一个软链（见下方折叠块）
```

> [!important] 先记住这一句就够用
> 要持久化，就写进 `/etc/sysctl.d/*.conf`，文件名排在 `99` 之后（例如 `100-local.conf`），然后执行 `sysctl --system`，再用 `sysctl <参数名>` 复查。

> [!info]- 进阶：`/etc/sysctl.conf` 与 `/etc/sysctl.d/` 到底谁先谁后
...
> [!check] 读完 1.7 应该能回答
> `sysctl -w` 改过的参数重启后还在吗？想持久化该写到哪里，怎么写才不会被 `99-sysctl.conf` 盖掉？

## 2. 生产实践

### 2.1 环境与版本基线

这套笔记里所有「默认值」都要落到本机实测上，先建基线再动手。这组命令和 [[01_实验环境与操作纪律]] 的基线表是同一套，这里额外关心的是「根分区到底是谁」，因为后面所有启动链排障都要靠它：
...
stat -fc %T /sys/fs/cgroup           # 验证：cgroup 版本，cgroup2fs 即 v2（阶段 3/4 会继续用）
...
**进阶（第一遍可以跳过）**：下面两样属于「知道有这条路、用到再查」的层级，不影响你完成一次正常的内核升级。
...
### 2.3 崩溃取证：kdump、`vmcore` 与 `crash`【进阶：第一遍可以跳过】

> [!tip] 第一遍只需要知道这一段
> 内核 panic 之后，现场在内存里、重启即失；`kdump` 的作用就是在 panic 时**自动把内存镜像写下来**，产物在 `/var/crash/` 里叫 `vmcore`。所以看到机器异常重启，第一反应是先去 `/var/crash/` 和带外控制台取证，而不是立刻重启。
> 下面这些配置命令与 `crash` 分析需要 debuginfo 和内核崩溃调试经验，属于阶段 11 生产运维的深水区，第一遍读完上面这句话就可以过。
...
这张表是全篇的「后悔药」清单。**第一遍只需要记住前四条**——它们分别对应启动链最常出事的位置：`fstab`、内核升级、`/boot` 空间、驱动是否存在。
...
> **最低成本复现**：在虚拟机上把 `grub.cfg` 改名再重启，会落到 `grub>`（GRUB 本体可用、读不到配置）；把 `/boot` 所在磁盘从虚拟机里移除或改掉启动顺序，会落到 `grub rescue>`（连正常模块都没加载）。改动前先打快照。
...
> **最低成本复现**：在 GRUB 里给内核行加 `rd.break`，就能停进同一个 initramfs shell（视角略有差异：真根此时挂在 `/sysroot`）；要模拟「设备真的不见了」，可以摘掉虚拟机的一块数据盘，再让 `fstab` 去挂它。
...
> **最低成本复现**：按第 4 节实验 3 往 `/etc/fstab` 加一行假 UUID 的挂载项，先 `mount -a` 确认报错，再重启。这是所有复现方式里最接近真实故障、也最容易把机器搞失联的一种，务必先打快照。
...
3. **网络类**：`network-online.target` 依赖链被当成硬依赖时，会把开机变成「等网络」；服务只应 `Wants=network-online.target` 而不是 `Requires=`，细节见 [[08_systemd与服务管理]]。
...
> **最低成本复现**：和 3.3 节同源——同一个坏挂载项，只是这次把启动过程完整看完，观察那条 `A start job is running for ...` 的 90 秒超时；网络类等待则可以在实验机上启用 `NetworkManager-wait-online.service` 后观察。
...
> **最低成本复现**：第 4 节实验 5。没有实验机时，先做两件只读的事：`ls /var/crash/` 看本机历史上抓没抓到过 `vmcore`，`journalctl --list-boots` 看日志有没有持久化。
...
> **最低成本复现**：把同一个 `sysctl` 参数分别写进 `/etc/sysctl.d/50-a.conf` 与 `/etc/sysctl.d/99-b.conf`，给不同的值，`sysctl --system` 之后看谁生效、`sysctl -a` 里显示什么。
...
> **最低成本复现**：第 4 节实验 4。没有快照实验机时不要练——这条路径会改到认证文件与 SELinux 标签，在生产上属于安全事件。

## 4. 动手验证

> [!warning] 前置纪律
> 六个实验**按风险从低到高排列，建议按顺序做、不要跳着做**：实验 0 只读；实验 1、2 可回滚；实验 3、4、5 会真的把机器弄进故障状态（写坏 `fstab`、重置 root 密码、主动触发 panic），**只在可快照的实验机上做，动手前先打快照**。生产机上只做实验 0 这类只读观测（`cat`、`ls`、`journalctl`、`systemd-analyze`）。

> [!example]- 实验 0：只读观测你自己机器的启动过程（零风险，先做这个）
> ```text
...
> journalctl -b -k | head -50           # 验证：本次启动的内核日志长什么样（对照 1.6 的失败现象）
> lsblk -o NAME,SIZE,TYPE,FSTYPE,MOUNTPOINT,UUID   # 验证：根分区与它现在的 UUID（`root=` 的依据）
> lsinitrd | head -30                   # 验证：initramfs 里到底装了什么（RHEL 系；Debian 系用 lsinitramfs）
> ```
> 预期：`systemd-analyze time` 的四段加起来接近总耗时；`blame` 里的高耗时单元与 `critical-chain` 里 `@` 后面标注的时间要能对上，对不上的部分通常是「并行等待」——这正是只看 `blame` 会误判的原因。
...
> 风险：中，要进 GRUB 改启动项并重启两次，但只加一个日志参数，不动挂载与内核。
> 耗时：约 30 分钟（含两次重启）。
> 回滚：把 `/etc/default/grub` 改回去，重新生成配置再重启。

> [!example]- 实验 2：`sysctl` 三种改法的作用域
> ```text
...
> 风险：高，会真的让机器进不了正常启动，远端机器可能直接失联。
> 耗时：约 40 分钟（含一次故障启动与恢复）。
> 回滚：恢复 `fstab.bak` 那一份，或删掉新增行。

> [!example]- 实验 4：`rd.break` 重置 root 密码并被 SELinux 拦一次
> ```text
...
> 风险：高，绕过认证改 root 密码，并会改到 SELinux 标签；在生产上这属于安全事件。
> 耗时：约 30 分钟（含两次重启与一次全盘重打标签）。
> 回滚：把 root 密码改回原值，删除 `/.autorelabel`。

> [!example]- 实验 5：触发一次 panic，验证 kdump 是否真的能抓到现场
> ```text
...
> 风险：最高，主动让内核崩溃，可能丢失数据；kdump 没配好的话连现场都没有。
> 耗时：约 40 分钟（含重启与 `vmcore` 检查）。
> 回滚：清理 `/var/crash` 中的大文件，确认预留内存与业务内存需求是否需要调整。

## 5. 要点自测

题目前带 **【进阶】** 的，对应笔记里的进阶小节，答不上来不影响阶段 2 过关；其余题目要求能盖住答案脱稿讲 3~5 分钟。
...
> [!question]- 【进阶】`sysctl -w` 改过的参数重启后会怎样？想持久化该写到哪里？`/etc/sysctl.conf` 与 `/etc/sysctl.d/` 谁先谁后？
...
- [ ] 能区分启动参数与运行时参数，独立完成一次启动参数的持久化修改并用 `/proc/cmdline` 验证
- [ ] 能独立完成一次 `sysctl` 持久化修改，并用 `sysctl`、`sysctl --system` 验证
- [ ] 【进阶】能讲清 `/etc/sysctl.conf` 与 `/etc/sysctl.d/` 的加载顺序与覆盖关系
...
- [ ] 能按「升级前准备 → 升级中保留旧内核 → 重启后验证 → 回滚」走完一次内核升级流程
- [ ] 【进阶】能讲清 `kexec` 与 Livepatch 各自解决什么问题、边界在哪
- [ ] 【进阶】能讲清 `kdump` 的工作原理、`vmcore` 取用路径与 `crash` 的初步分析顺序

> 推荐扩展阅读：`man 7 bootup`、`man 7 systemd.special`、`man 5 sysctl.d`、`man 8 dracut`、`man 8 kdumpctl`（RHEL 系）、`man 5 crontab`（阶段 8 会用到），以及发行版官方文档中「Managing the kernel / Working with GRUB 2」与「Kernel crash dump」两章。

## 验证进度

本文的结论来自 man 页与发行版官方文档，**尚未在实验机上逐条复现**，所以文中所有「预期输出」目前都是文档值。标记约定：

- **✅ 已实测**：在真实实验机上跑过、输出与文中一致，并回填了实测环境（发行版 / 内核版本 / systemd 版本）。
- **⚠️ 文档值**：来自 man 页或官方文档、未实测；本机跑出来不一致时以本机输出为准，并回来更新这一条。

当前进度：**全篇为 ⚠️**。建议先补一份版本基线（`cat /etc/os-release`、`uname -r`、`systemd --version`），再从第 4 节的实验 0 开始逐个做，把实测输出、耗时与踩坑回填到对应小节，然后按小节把这里改成 ✅。
...
- [x] 跨笔记引用都用 wikilink；跨目录引用用路径写法；指向尚未创建的阶段笔记（[[08_systemd与服务管理]]、[[10_性能分析与排障]]）的链接与 [[Linux/00_简介|00_简介]] 的规划保持一致，目前在 Obsidian 中显示为未解析属预期
- [x] [[Linux/00_简介|00_简介]] 的阶段 2 已有「对应笔记」链接与 checklist 条目
- [x] 全篇按难度分了两层（【核心】/【进阶】），第 4 节的实验按风险从低到高排列，并标注了风险、耗时与回滚
- [x] 每个排障小节都给了「最低成本复现」或明确的「没有快照机就不要练」
- [ ] 每条命令都附了「验证什么」与预期输出，并在真实实验机上跑过
- [ ] 第 4 节的六个实验（实验 0–5）全部实操完成，把实测输出、耗时与踩坑回填到对应小节
