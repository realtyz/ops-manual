...
> 本笔记对应 [[Linux/00_简介|Linux 大纲]] 的「阶段 4：存储、文件系统与 IO」。目标：能讲清「一块盘到目录」之间的每一层，能处理磁盘满、inode 满、只读文件系统、IO 变慢这四类高频故障，并且能在换盘、扩容、重建这些动作上有确定的流程。
>
> 关联复习：[[01_启动流程与内核]]（fstab 写错 = 起不来，挂载单元与 rescue）、[[02_进程与信号]]（D 状态与句柄上限）、[[03_内存管理与OOM]]（Page Cache 与脏页回写）、[[09_性能分析与排障]]（IO 瓶颈的分层定位）。
>
...
> 本文结论来自 man 页、内核文档与发行版文档，**尚未在实验机上逐条复现**。
> 建议顺序：先取基线（`lsblk`、`df -hT`、`df -i`、`lsblk -d -o NAME,ROTA`），再按第 4 节的实验（全部基于 loop 设备，可在任意实验机上安全复现）逐个做，最后核对文末「自检清单」。
...
> - **「有空间却写不进去」有两类**：inode 满了（`df -h` 有余、`df -i` 用尽，小文件场景），以及 ext4 为 root 保留的 5% 空间（`df` 显示可用但你写不进去）。**判据是 `df -h` 与 `df -i` 一起看。**
> - **`df` 与 `du` 对不上有四类原因**：已删除但被进程持有的文件（`lsof +L1`）、被挂载覆盖的目录、文件系统保留块、稀疏文件/延迟分配。前三类都是「空间真的被占了但 `du` 看不见」。
...
> 第 4 节的实验全部用 loop 设备搭建，**不需要真盘、不会碰生产数据**，建议至少把「inode 耗尽」「df/du 不一致」「LVM 在线扩容」三个做完。
...
- **`lsblk` 显示的是「设备/分区/挂载」的树**，`df` 显示的是「文件系统」的容量；同一个文件系统可以挂到多个点（bind mount），一个设备也可以没挂载。
- **GPT 与 MBR**：MBR 最多 4 个主分区、单盘上限 2TB；GPT 支持 128 个分区（Linux 默认）与更大容量，现代系统（UEFI 或 >2TB 的盘）一律用 GPT。
- **一律用 UUID 或 LABEL 挂载，不用 `/dev/sdX`**：设备名会随插槽、控制器、云盘挂载顺序变化，这是「重启后挂载错盘」的经典原因（见 1.3 与 [[01_启动流程与内核]]）。
...
| `discard`（SSD） | 删除时立即下发 TRIM | 见 1.12：通常更推荐 `fstrim.timer` 定期批量执行 |

几个实操要点：

- **改完 fstab 一定要先验证再重启**：`mount -a` 能立刻暴露语法与设备名错误；`findmnt --verify` 可以专门校验 fstab 的可挂载性。这是「重启起不来」的头号原因（参见 [[01_启动流程与内核]]）。
...
把上面的判断合成一条排查路径，遇到「写不进去」时按它走一遍就够了：

```mermaid
flowchart TD
  A["写入失败：No space left on device"] --> B{"df -h 已用满了吗"}
  B -- "是" --> C{"du 的统计能对上吗"}
  C -- "对得上" --> D["真的写满：清理过期数据或在线扩容"]
  C -- "对不上" --> E["四类不一致：已删除但被进程持有 / 挂载覆盖 / 保留块 / 稀疏与延迟分配"]
  B -- "否" --> F{"df -i 已用满了吗"}
  F -- "是" --> G["inode 耗尽：清理小文件，长期改 XFS 或用更小的 bytes-per-inode 重建"]
  F -- "否" --> H{"配额到顶了吗"}
  H -- "是" --> I["配额治理：调整配额或让租户清理"]
  H -- "否" --> J["继续查：ext4 保留块、文件系统被重挂为只读、写入路径其实落在别的挂载点"]
```

图中的每一步都对应一条命令（见 3.1 与 3.2），**这也是面试里最容易被追问细节的一道题**：能把「先看哪个、再看哪个、每步用什么证明」讲清楚，比背一堆命令更能说明经验。

### 1.6 文件系统修复：为什么必须卸载，怎么修才安全
...
> 正确顺序：取证（`dmesg -T`、`journalctl -k`）→ 切走业务/停止写入 → 卸载 → 修复 → 只读挂载核对 → 恢复写。同时要判断根因是**介质故障**（见 1.12）还是**元数据损坏**，前者必须先换盘。
...
老一代的 `cfq`/`deadline`/`noop` 在多队列（blk-mq）架构下已不再提供，**在 RHEL 8/9 上看到这四个才不会觉得奇怪**；RHEL 9 文档给出的默认值是「NVMe 用 `none`、其它块设备用 `mq-deadline`」，改之前先确认本机现状。
...
网络存储把「本地 IO 快慢」变成了「本地 IO + 网络 + 对端存储」的联合问题，其中最坑的是**挂载点卡死导致进程进 D 状态**（与 [[02_进程与信号]] 的 3.1 直接相关）。
...
| XFS | 挂载参数 `uquota`/`gquota`/**`prjquota`**；较新内核也支持挂载后用 `xfs_quota -x -c 'enable'` 打开 | `xfs_quota -x -c 'report -h' /mnt`、`xfs_quota -x -c 'project -s -p <path> <id>' /mnt`、`limit` |

```text
mount | grep -E 'quota'                 # 验证：当前挂载是否启用了配额
...
这两类上限与存储强相关，又经常被漏掉（与 [[02_进程与信号]] 的 1.7、1.8 交叉看）：
...
2. **看是不是配额**：`repquota -a`（ext4）或 `xfs_quota -x -c 'report -h'`（XFS）——配额到顶同样报「无空间」。
...
5. **别忘了非磁盘因素**：Page Cache 回写（[[03_内存管理与OOM]] 1.3）、网络存储链路（1.11）、阵列卡缓存策略与电池状态。
...
5. **只读挂载核对数据**，确认无误再恢复读写；如果是介质故障，先换盘再恢复（见 1.12）。

### 3.5 挂载点卡死：进程堆积在 D 状态

> [!tip] 第一反应不要是「批量 `kill -9`」
> 卡在不可中断等待上的进程收不到信号（见 [[02_进程与信号]] 3.1），杀不掉也不该杀——要做的是恢复 IO 通路。
...
> [!tip] 第一反应不要是重装
> 这是**最常见的可修复故障之一**（[[01_启动流程与内核]] 3.3 的经典场景）。
...
> [!warning] 前置纪律与安全设计
> 下面的实验**全部基于 loop 设备（用一个普通文件模拟磁盘）**，不需要真盘、不会碰生产数据；仍然建议在可快照的实验机上做。唯一有破坏性的是 `blkdiscard`（本篇只在 1.12 说明用法，不放进实验）。
...
> [!example]- 实验 4：LVM 全链路（创建 → XFS → 在线扩容 → 快照）
> ```text
...
> mkfs.xfs /dev/vglab/lvdata && mkdir -p /mnt/lvmdata && mount /dev/vglab/lvdata /mnt/lvmdata
...
> ```
> 预期：`lvextend -r` 一步完成「扩 LV + 扩文件系统」，`df` 立即可见；快照的 `data_percent` 随原卷写入上升——**这正是「快照会涨、会写满、不能当长期备份」的原因**。
> 环境：任意发行版，需要 `lvm2` 与 root。**注意**：`-r` 会调用 `xfs_growfs`（参数是挂载点），这就是 1.7 里强调的那个差异。
...
> iostat -x 1 30
> # 终端 B：跑随机读与顺序写两组基线
> fio --name=lab-randread --filename=/tmp/lab-disk0.img --ioengine=libaio --direct=1 \
>     --rw=randread --bs=4k --iodepth=32 --numjobs=2 --size=512M --runtime=30 --group_reporting
> fio --name=lab-seqwrite --filename=/tmp/lab-disk0.img --ioengine=libaio --direct=1 \
...
> 环境：任意发行版（`fio` 需安装）；**loop 文件的数字不代表真实磁盘性能**，本实验练的是「怎么读指标、怎么记录条件」。
> 回滚：无（`--filename` 指向临时文件；若担心，先按实验 1 重建该文件）。
...
> ```
> 预期：`e2fsck -n` 只报不改；`xfs_repair` 对已挂载设备会直接拒绝——**这正是 1.6 的核心约束**：修复必须在卸载状态下做。
...
> - **保留块 / 配额**：ext4 默认 5% 只给 root；配额到顶也会报「无空间」。
...
- [x] 跨笔记引用都用 wikilink；跨目录引用用路径写法；指向尚未创建的阶段笔记（[[09_性能分析与排障]]）的链接与 [[Linux/00_简介|00_简介]] 的规划保持一致，目前在 Obsidian 中显示为未解析属预期
...
- [ ] Mermaid 图（「写不进去」的排查路径）已在 Obsidian 里预览过，能正常渲染
