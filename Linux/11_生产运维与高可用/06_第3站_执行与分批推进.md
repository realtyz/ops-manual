...
>
> **实测状态**：实测环境 **Ubuntu 24.04.5 LTS（VMware 虚拟机）/ 内核 `6.8.0-139-generic` / systemd 255 / 4 vCPU / root 可用**。**已实测**：① 单机验证与摘流量用到的命令（`systemctl show`、`ss -lnt`、`ss -tn state established '( sport = :PORT )'`、`journalctl`、`curl -w`）——用一个自建临时 unit 与 `python3 -m http.server` 起在**端口 21080**（本章分配到的 21000–21199 区间）上真跑，输出见 1.1、1.2；② **串行 vs 并发的耗时对比**——5 次「reload 一次耗时 1 秒的服务」实测串行 **5100 ms**、`xargs -P 4` **2058 ms**（见 2.2）；③ `timeout` 的退出码语义（超时 **124**、正常 **0**）；④ 执行取证「改前 / 改后」（`vm.swappiness` 60 → 10 → 60）与变更后错误计数（见第 3 节）。全部临时 unit 与监听端口已清理。
>
> **未实测**：多机批量执行、摘流量、Ansible——**本实验环境是单台虚拟机，没有第二台机器、没有负载均衡**，按方法给出；`ansible` 本机未安装（`command -v` 返回空），命令写法来自官方文档。
...
**本机没有 `:8443` 这样的业务端口，所以用一段自建的临时服务把这两条命令跑通**——重点是命令的**形态与判据**，不是端口号。用 `python3 -m http.server` 在**端口 21080** 上起一个最小 HTTP 服务（本章分配到的端口区间是 21000–21199），先用 `ss -lnt` 确认它在听，再用 `ss -tn state established` 与 `curl -w` 各取一次证据：

```bash
mkdir -p /tmp/11a-exec/www && printf 'ok\n' > /tmp/11a-exec/www/healthz
python3 -m http.server 21080 --directory /tmp/11a-exec/www &
ss -lnt | grep -E ':21080'                # 验证：服务真的在听（不是"我启动了"）
ss -tn state established '( sport = :21080 )' | head
                                          # 验证：此刻有多少条存量连接到这个端口
curl -sS --max-time 3 -o /dev/null -w '%{http_code} %{time_total}\n' http://127.0.0.1:21080/healthz
                                          # 验证：本机探活（响应码 + 耗时）
```
...
```text
$ ss -lnt | grep -E ':21080'
LISTEN 0      5            0.0.0.0:21080      0.0.0.0:*

$ ss -tn state established '( sport = :21080 )'
Recv-Q Send-Q Local Address:Port Peer Address:PortProcess

$ echo "established_count=$(ss -tn state established '( sport = :21080 )' | tail -n +2 | wc -l)"
established_count=0

$ curl -sS --max-time 3 -o /dev/null -w '%{http_code} %{time_total}\n' http://127.0.0.1:21080/healthz
200 0.003789

$ curl -sS --max-time 3 http://127.0.0.1:21080/healthz
ok
```

这段输出里有两个容易被忽略的细节：**① 「在听」和「有连接」是两个独立事实**——`LISTEN` 有 1 条、`established` 是 0 条，短连接（`curl` 一次请求就关）在 `ss -tn` 里几乎抓不到，所以**「摘流量前先数存量连接」要用一个能持续观察的窗口**（循环采样或长连接），单次 `ss` 拍到的 0 不代表没有存量；**② `curl -w` 的 `%{time_total}` 是 0.003789 秒（3.8 ms）**——本机回环探活就是这个量级，**把它当基线的意义是「业务路径的耗时应当远大于它」**；如果一次真实业务探活也是 3.8 ms 量级，那多半探的不是业务路径，而是某个静态的健康检查端点。
...
单机验证三件事在实验机上的实测（以一个自建临时 unit + 端口 21080 上的最小 HTTP 服务为例）：

```text
$ systemctl show lab11a-tick -p ActiveState -p SubState -p MainPID -p NRestarts -p ExecReload
MainPID=19777
NRestarts=0
ExecReload={ path=/bin/sleep ; argv[]=/bin/sleep 1 ; ignore_errors=no ; ... }
ActiveState=active
SubState=running
...
`systemctl show` 的输出要**读三个字段而不是一个**：`ActiveState=active` 只说「在跑」，`SubState=running` 说「处于正常运行态（而不是 `auto-restart`/`start-pre`）」，`NRestarts=0` 说「没在反复重启」。**只看 `ActiveState` 会漏掉「服务正在崩溃重启循环中」这个最常见的假健康**。
...
**本机没有第二台机器可以 `ssh`，所以把「主机清单」全换成本机、把「reload 的目标」换成一个自建的临时 unit**（`ExecReload=/bin/sleep 1`，让每一次 reload 固定耗时 1 秒，好把并发的收益量出来）。这样串行与并发两条路径都能真跑：

```bash
printf 'localhost\nlocalhost\nlocalhost\nlocalhost\nlocalhost\n' > /tmp/11a-exec/hosts.txt
s=$(date +%s%N)
while read -r h; do systemctl reload lab11a-tick; printf '%s is-active=%s\n' "$h" "$(systemctl is-active lab11a-tick)"; done < /tmp/11a-exec/hosts.txt
e=$(date +%s%N); echo "serial_ms=$(( (e-s)/1000000 ))"
                                          # 验证：串行 5 台的总耗时
s=$(date +%s%N)
xargs -P 4 -I{} sh -c 'systemctl reload lab11a-tick; printf "%s is-active=%s\n" "{}" "$(systemctl is-active lab11a-tick)"' < /tmp/11a-exec/hosts.txt
e=$(date +%s%N); echo "parallel_ms=$(( (e-s)/1000000 ))"
                                          # 验证：并发度 4 执行同样 5 个目标的总耗时
```
...
**这组数字要连条件一起读**：每次 reload 固定 1 秒，5 次串行 = **5100 ms**（≈ 5 × 1 秒 + 启动开销）；`-P 4` 时 = **2058 ms**（≈ ⌈5/4⌉ × 1 秒 + 启动开销）。**并发的收益是有上限的**——它不是 5 倍，而是 ⌈N/并发度⌉ 倍；把 `-P` 从 4 提到 8，这次的 5 个目标也不会更快（因为只有 5 个）。所以 `-P` 的取值应当由「单台动作耗时 ÷ 可接受的总窗口」反推，而不是「越大越好」。

**并发版还有一个看不见的代价**：上面两次输出的 `is-active=active` 完全一样，**根本无法从输出判断哪一台是先做的、哪一台失败过**——串行版至少能靠「日志停在哪一行」定位。这就是「并发度是效率与可定位性的交换」的实证。
...
`timeout` 的退出码语义要实测记牢（本机实测）：

```text
$ timeout 2 sleep 5; echo "timeout-2s-on-sleep5 exit=$?"
timeout-2s-on-sleep5 exit=124
$ timeout 30 true; echo "timeout-30s-on-true exit=$?"
timeout-30s-on-true exit=0
```

**超时退出码是 124，不是 1，也不是 137**。这一点在批量脚本里很关键：`124` 专指「被 `timeout` 掐掉」，而 `1` 是「被执行的命令自己失败」、`137` 是「被 `SIGKILL` 杀掉」。所以脚本里可以据此分开处理——**124 的机器单独记下来重试，其余非 0 的机器按失败处理**：
...
case $rc in
  0)   echo "OK" ;;
  124) echo "TIMEOUT: $host 单独处理，不阻塞其余批次" ;;
  *)   echo "FAILED(rc=$rc): $host 停止扩批并查原因" ;;
esac
                                          # 验证：三种结局走三条处置路径
```
...
sysctl -p /etc/sysctl.d/99-lab.conf
                                          # 验证：只重放这个文件（不要用 --system，见子笔记 05 的说明）
after=$(sysctl -n vm.swappiness)
                                          # 验证：改后的值
echo "swappiness: $before -> $after"
                                          # 验证：把这对值抄进执行记录
rm -f /etc/sysctl.d/99-lab.conf; sysctl -w vm.swappiness="$before"
                                          # 验证：实验做完立刻还原，并再读一次确认
```
...
vm.swappiness = 10
swappiness: 60 -> 10
$ rm -f /etc/sysctl.d/99-lab.conf; sysctl -w vm.swappiness=60 >/dev/null; sysctl -n vm.swappiness
60
```
...
这里的 `-u <unit>` **不能省**。本机实测过两种写法，差距很大：

```text
$ journalctl -u ssh --since "10 min ago" --no-pager | grep -icE 'error|fail'
0

$ journalctl --since "10 min ago" --no-pager | grep -icE 'error|fail'
93
```

同一个 10 分钟窗口，**限定单元是 0 条，不限定单元是 93 条**——那 93 条几乎全来自这台机器上别的服务与别的实验（这个实验机是共用的）。**不限定单元的错误计数不是「变更后的新增报错」，而是「整台机器的噪声」**，拿它当验收判据只会得到假警报，进而让人对告警麻木。要把口径写进变更单：**「`journalctl -u <unit> --since <变更开始时刻>` 的 `error|fail` 计数为 0」**。
...
> [!example]- 实验 3：在实验机上做一次分批重载（实验机实测，需要 root）
> **怎么做**：本机没有 `nginx`，也没有第二台机器，所以用**自建的临时 unit** 代替业务服务，用**全本机的主机清单**代替多机清单：
> ① 写一个 `/etc/systemd/system/lab11a-tick.service`（`ExecStart=/bin/sleep infinity`、**`ExecReload=/bin/sleep 1`**——让每次 reload 固定耗时 1 秒，好把并发的收益量出来）；
> ② 写一份 5 行的「主机清单」，全是 `localhost`；
> ③ 分别用**串行 `while read`** 与 **`xargs -P 4`** 两种方式各 reload 5 次，记录耗时与每台的 `is-active`；
> ④ 清理：`systemctl stop` + 删 unit + `daemon-reload` + `reset-failed`。
> **实测结果**：串行 **5100 ms**、`xargs -P 4` **2058 ms**；两次的 `is-active` 输出都是 5 个 `active`。**并发快了约 2.5 倍（≈ ⌈5/4⌉），但输出的 5 行完全一样**——**失败时更难定位是哪一台出的问题**，这个对比正是「并发度是风险交换」的直接证据。
> **风险**：低（只操作自建 unit、只在本机）。**不要在生产机上做**；也不要在实验机上有真实业务流量时做。
> **耗时**：约 20 分钟。
> **怎么退回去**：服务停在自建 unit 上，`stop` 并删掉 unit 即可；清理后确认 `ss -lnt` 里没有多余的监听、`systemctl list-units --failed` 里没有 `lab11a-tick`。
