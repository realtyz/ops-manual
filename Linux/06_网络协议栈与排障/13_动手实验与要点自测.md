...
>
> **实测声明**：下面的实验输出全部是在实验机上**实测粘贴**的。环境：**WSL2 上的 Ubuntu 24.04.4 LTS，内核 `6.6.87.2-microsoft-standard-WSL2`，cgroup v2，24 vCPU，虚拟网卡驱动 `hv_netvsc`**（*虚拟网卡不是物理网卡，`ethtool` 的部分字段与计数器不能代表真实设备*）。
> 破坏性/干扰性实验（半连接队列、`netem` 注入、MTU 不一致）全部在**独立 network namespace** 里做；实验结束已删除命名空间，宿主网络栈未改动（实测 `net.ipv4.tcp_syncookies` 仍为 `1`）。
...
| 实验 6 | MTU 不一致：小包通、大包丢（同一拓扑） | **可快照实验机** | T3 | N6 | 08 |
...
> ```bash
> # 准备：确认工具与权限（实验 3 伪造 SYN 需要 CAP_NET_RAW）
> command -v ip python3 ss nstat sysctl tc ping
> ip netns list                       # 验证：是否已有 srv/cli（有就先清理）
> sysctl net.ipv4.tcp_syncookies      # 验证：宿主默认值（实验 3 只在命名空间内临时改，宿主不变）
> ```
> **实验与正文的对应**：实验 1、2、3 → 子笔记 05 的两个队列；实验 4 → 子笔记 07 的 `TIME_WAIT`；实验 5 → 子笔记 10 的抓包与故障注入；实验 6 → 子笔记 08 的 MTU；实验 7 → 子笔记 06 的 `ss -ti` 与子笔记 11 的设备观测。
> **没有配套实验的小节**：子笔记 02 的路由与邻居、04 的 DNS 解析链、09 的转发与 conntrack、11 的主机侧调优——它们依赖真实网络、DNS 环境或物理网卡，只给只读的观测命令。
...
sysctl net.ipv4.tcp_max_tw_buckets net.ipv4.tcp_rmem net.ipv4.tcp_wmem \
       ...
       net.netfilter.nf_conntrack_max net.netfilter.nf_conntrack_count
                                     # 验证：上限类参数（缓冲、端口、conntrack）
ethtool -i eth0; ethtool -g eth0; ethtool -l eth0; ethtool -k eth0
                                     # 验证：驱动与固件、环缓冲、队列数、offload
ip -s link                           # 验证：每张网卡的收发包与 errors/dropped
nstat -az > /tmp/nstat.baseline      # 验证：计数器留档（后面才有「变差了」的参照）
```

**预期**：得到一份能回答「有几张网卡、地址与路由怎么走、监听哪些端口、关键参数是多少、当前状态分布如何」的基线卡；`nstat` 快照落盘，供后续对比。

**环境**：任意发行版，以 root 或带 `sudo` 执行；`ethtool` 对虚拟网卡可能缺少部分字段（实测 `firmware-version: N/A` 属正常）。
...
**这一屏就是子笔记 05 与 12 的全部证据**：`Recv-Q > Send-Q`（队列满）+ `ListenOverflows` 增长（内核在丢 SYN）+ 客户端 `SYN-SENT`（应用永远不知道这条连接存在过）。

> 说明：上面的输出取自**隔离命名空间的同一实验**（地址是 `10.0.0.1`/`10.0.0.2`）；本脚本改用 `127.0.0.1` 是为了免建拓扑。
> loopback 版本已在另一台 WSL2 Ubuntu（iproute2 6.1）上复跑：`LISTEN 2 1 127.0.0.1:9999`、第 3 条起 `TimeoutError`、`TcpExtListenOverflows`/`ListenDrops` 同步增长到 **9**（比客户端条数多，因为失败的连接会重传 SYN）、收尾后端口释放且无残留进程——**结论一致，只有地址列不同**。

**风险**：低。只占用本机 `9999` 端口与少量进程。

**耗时**：约 15 分钟。

**回滚**：命令里已 `kill %1 %2`；确认 `ss -lnt | grep 9999` 无输出即可。
...
**回滚**：`kill %1`；确认 `ss -lnt | grep 9997` 无输出。

## 4. 实验 3：半连接队列与 syncookies（隔离命名空间 + 伪造源 SYN）

先搭一套**完全隔离**的两命名空间拓扑（后面实验 5、6、7 复用），再往里灌 SYN：
...
cat > /tmp/lab_srv2.py <<'PY'   # ② srv 里起一个 listen(1) 且不 accept 的服务端
...
cat > /tmp/lab_syn2.py <<'PY'   # ③ cli 里用原始套接字发带伪造源地址的 SYN（只在本命名空间内，出不了这台机器）
...
**结论**：`TIME_WAIT` 的时长是**内核写死的 60 秒**，与 `tcp_fin_timeout`（实测 60、管 `FIN_WAIT2`）无关，也没有任何 sysctl 可调——**这就是子笔记 07 那条「面试误解」的实测依据**。
...
**回滚**：脚本自行结束；确认 `ss -tan | grep 44445` 无输出。
...
**实测输出**（注入前基线 `rtt min/avg/max/mdev = 0.016/0.023/0.031/0.007 ms`。注入 50ms 后）：
...
```bash
cat > /tmp/lab_acc.py <<'PY'   # ① 服务端脚本：accept 之后保持连接，不回数据也不关闭
import socket, sys, time
addr, port, backlog = sys.argv[1], int(sys.argv[2]), int(sys.argv[3])
s = socket.socket()
s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
...
cat > /tmp/lab_cli3.py <<'PY'   # ② 客户端脚本：连上之后什么都不发
import socket, sys, time
...
ip netns exec srv python3 /tmp/lab_acc.py 10.0.0.1 9998 1 &   # ③ 先起服务端，再起客户端，最后在服务端看这条连接
sleep 1
ip netns exec cli python3 /tmp/lab_cli3.py 10.0.0.1 9998 &
sleep 2
...
**逐字段读法见子笔记 06 的表**：`minrtt:0.011` 就是这条路径的延迟基线（veth 直连）；没有 `retrans`/`lost` 字段说明无重传；`app_limited` 说明这条空闲连接没被应用喂满——**「慢」不在网络上**。
...
ip netns list                                             # 验证：srv/cli 已消失
rm -f /tmp/lab_srv2.py /tmp/lab_syn2.py /tmp/lab_tw.py /tmp/lab_acc.py /tmp/lab_cli3.py
ss -lnt | grep -E '9997|9998|9999|44445' || echo '实验端口已全部释放'
```

**预期**：清理后 `ip netns list` 里不再有 `srv`/`cli`，四个实验端口全部释放（veth 会随命名空间一起消失，实测无残留）。

> [!warning] 清理后要做的复查
> ① `sysctl net.ipv4.tcp_syncookies` 应与实验前一致（实测宿主始终为 `1`，因为改动只发生在命名空间内）；② `tc qdisc show` 无 netem 残留；③ 没有遗留的 `python3` 后台进程（`pgrep -af lab_ | head`）。
...
- [x] 全部实验输出为实测粘贴，并标注了环境（WSL2 / Ubuntu 24.04 / 内核 6.6）与回滚方式
- [x] 统一清理命令覆盖 namespace、veth、临时脚本与实验端口
- [x] 自测题覆盖本章十个核心结论，且每题都给出「第一反应不要是什么」
- [x] 未实测的工具（`tcpdump`/`mtr`/`iperf3`/`conntrack`）在引用处已标注
- [ ] 在**物理网卡**上补一次基线（`ethtool -S/-g/-l`、`/proc/interrupts` 的队列分布）
- [ ] 在真实跨网链路（有丢包/跨地域）上复测 `cubic` vs `BBR`，把量化收益回填到子笔记 06
