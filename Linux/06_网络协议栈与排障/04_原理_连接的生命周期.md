...
flowchart LR
  ...
  D --> E{"resolv.conf 指向谁"}
  E -- "127.0.0.53" --> F["systemd-resolved 存根：缓存、search/ndots 与 LLMNR/mDNS"]
  E -- "真实 DNS" --> G["直接查上游"]
  H["dig / nslookup"] -.->|"绕过 NSS，直接发 DNS 查询"| G
```

读法：**应用只走上面那条路**，即 `getaddrinfo()` 经 NSS 逐项去问；`dig` 走下面那条虚线，两者可以给出完全不同的结果，而且都不算「坏」。
...
1. **本机配置放大**：因为 `search` 域多、`ndots` 也大，所以一次解析会按后缀数量被放大成多次查询，而每一次都要等超时才轮到下一个。查询次数是可以事先算的：名字里的点数少于 `ndots` 时，先按 `search` 域逐个尝试，最后才查原名，因此次数 = `search` 域数量 + 1。容器与 Kubernetes 注入的 `ndots:5` 是这类问题的常见来源：`foo.bar` 这种带一个点的名字会**先试 `search` 列表里的每个后缀**，全部 NXDOMAIN 之后才查原名。
...
UDP 没有重传、没有拥塞控制（除非应用自己实现），所以**「没有重传」不等于「没有丢包」**。DNS（UDP 53，被截断时回落到 TCP）、SNMP 采集、以及自带重传与拥塞控制的 QUIC 都在这一条路上。UDP 服务的「偶发超时」经常查不出原因，就是因为**内核不统计 UDP 的丢包与重传**，只能靠应用层指标或抓包。
...
sequenceDiagram
  ...
  Note over P: LAST_ACK 之后进入 CLOSED
```
...
因此「消灭 `TIME_WAIT`」不是目标，**减少主动关闭的次数**才是。`tcp_max_tw_buckets` 溢出时内核会直接跳过 `TIME_WAIT` 关连接，代价正是破坏上面两条保护——它只是撞上这个上限之后的回退路径，不是优化手段。
...
典型原因：连接池泄漏、异常分支没有 `close()`、上游不读导致阻塞、依赖库 bug。定位路径是先找出进程，再看句柄是否单调增长，然后结合代码或上游分析，而不是「调参数」或「重启了事」。**它往往先表现为「连接池被打满」，而不是「服务报错」。**

## 常见误解

| 常见说法 | 事实 |
| --- | --- |
| 「`dig` 通了说明应用能解析」 | `dig` 绕过 NSS；应用走 `getaddrinfo()` 与 NSS，可能先查 hosts、经过 resolved 存根，也可能在别的命名空间里 |
...
| 「没有重传就没有丢包」 | 对 TCP 成立，对 **UDP 不成立**（内核不统计 UDP 的丢包与重传） |
| 「`cubic` 换 `BBR` 一定更快」 | 收益取决于链路是否存在随机丢包；换算法影响全机所有连接，且有公平性争议 |

## 要点自测

**问：`getent hosts` 与 `dig` 结果不一致，说明什么？**
答：说明两者走了两条不同的路径——`getent` 走 `getaddrinfo()` 与 NSS（其中 `files` 优先于 `dns`），而 `dig` 直接发 DNS 查询。差异可能来自 `/etc/hosts`、resolved 存根、`search`/`ndots` 后缀尝试，或「只显示一族地址」的记录选择。**「`dig` 通」不能证明应用能解析。**
