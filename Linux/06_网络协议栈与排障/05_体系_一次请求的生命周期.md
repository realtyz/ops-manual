...
一次请求从应用拿到一个名字开始，到连接断开、到转发设备与对端留下痕迹为止，中间穿过了本章所有机制。下面按这条时间线一次讲完，最后把贯穿多个阶段的五个核对对象（队列与上限、证据与计数、名字解析、主机内外、变更纪律）收拢在一起。
...
三者结果不一致是**信息**而不是故障：路径不同（NSS 与纯 DNS）、记录选择不同（一族与全部）、后缀规则不同（`search`/`ndots` 只作用于前者）。
...
| --- | --- | --- | --- | --- | --- |
| `via` | 下一跳地址 | 一个 IP | 它应当与本网段的网关配置一致 | `ip route`、`ip rule show` | 若不一致，则查更具体的路由或策略路由 |
| `dev` | 出口接口 | 接口名 | 多网卡机器上重点看它是不是预期的出口 | `ip -br addr` | 若走错了口，则先读 `ip rule show`，再看应用是否绑定 |
| `src` | 内核选定的源地址 | 一个 IP | 它应当是对端期望的那个地址 | 接口地址、`ip rule` | 若不对，则查源地址选择与应用绑定 |
| `uid` | 发起查询的 uid | 数字 | 它决定基于 uid 的规则是否命中 | `ip rule show` | 只有当规则里出现 uid 条件时这一步才有意义 |

| 你看到 | 含义 | 和谁一起看 | 转向哪一步 |
| --- | --- | --- | --- |
| `Network is unreachable` | 它说明没有任何路由能匹配这个目标 | `ip route`、`ip rule show` | 去补路由或修网关 |
| `ip neigh` 显示 `INCOMPLETE`/`FAILED` | 它说明同网段场景下二层没通 | `ip -s link`、VLAN/网桥配置 | 去查链路与对端，不要查路由表 |
| `ip -s link` 的 `errors`/`dropped` 增长 | 它说明链路层在丢包 | `ethtool -S` 的驱动计数 | 去查驱动、环缓冲与对端设备 |
| `ip route` 正常但走错出口 | 它说明策略路由提前把流量分流了 | `ip rule show` | 去核对规则的编号与条件 |

### 2.2 转发路径上的四处判定

包离开本机之后，若经过一个 Linux 转发节点（网关、容器节点、Kubernetes 节点），要依次经过四处判定：

```mermaid
flowchart LR
  A["进入本机"] --> B["PREROUTING<br/>DNAT"]
  B --> C{"目标是本机？"}
  C -- "是" --> D["INPUT 放行后交给本机进程，再由 OUTPUT 与 POSTROUTING 送出"]
  C -- "否" --> E["FORWARD 放行后进 POSTROUTING<br/>做 SNAT/MASQUERADE"]
```

| 判定点 | 载体 | 失败时的现场 | 转向哪一步 |
| --- | --- | --- | --- |
| 转发开关 | 内核参数 `net.ipv4.ip_forward` | 目的地址不是本机的包被丢弃或回 ICMP | 用 `sysctl net.ipv4.ip_forward` 读它的当前值 |
| 路由与邻居 | 路由表、邻居表 | `ip route get` 没有结果，或者邻居状态是 `FAILED` | 去查路由与二层 |
| 过滤 | `FORWARD` 链（nftables/iptables） | 包被丢弃，而主机侧的计数器可能完全干净 | 读 `nft list ruleset` |
| 连接跟踪 | conntrack 表 | **静默丢弃**，只在 `dmesg` 与统计里留下痕迹 | 读 `nf_conntrack_count/max` 与 `conntrack -S` |
...
| 你看到 | 含义 | 单位与判据 | 和谁一起看 | 转向哪一步 |
| --- | --- | --- | --- | --- |
| `ss -lnt` 的 `Recv-Q > Send-Q` | 全连接队列已满 | 个数；`Recv-Q` 是深度、`Send-Q` 是上限 | `nstat` 的 `TcpExtListenOverflows`/`ListenDrops` | 确认是 backlog 太小还是被 `somaxconn` 截断 |
| `Send-Q` 等于 `somaxconn` | 上限被全局参数截断 | 个数 | `sysctl net.core.somaxconn`、应用 `listen()` 值 | 先确认是哪一侧的瓶颈，再决定改谁 |
| `TcpExtListenOverflows`/`ListenDrops` 增量非 0 | 全连接队列溢出，**静默丢包** | 次数；必须取差值 | 客户端 `SYN-SENT` 状态 | 去查 `accept()` 的节奏与队列上限 |
| `TcpExtTCPReqQFullDoCookies` 增长、`ListenDrops` 不涨 | 半连接队列满但 syncookies=1 | 次数 | `sysctl net.ipv4.tcp_syncookies` | 客户端仍能连上，无需动作 |
| `TcpExtTCPReqQFullDrop` 与 `ListenDrops` 同步增长 | 半连接队列满且 syncookies=0 | 次数 | `sysctl net.ipv4.tcp_syncookies` | 把 `tcp_syncookies` 恢复为 1 |
| `ss -tan` 里 `SYN-SENT` 堆积 | 客户端发出去了却没人回 | 条数 | 服务端 `nstat`、中间设备与 conntrack | 若服务端计数干净，则去查中间设备 |

**证据齐全的标准是「两个维度对上」**：`ss` 的瞬时队列状态与 `nstat` 的累计计数增量。只看其中一个都可能错过偶发事件。这一节常见的三类误判（一次 `ss` 干净就排除队列、只调 `somaxconn`、`tcp_abort_on_overflow` 的取舍被当成开关）在判例篇逐条给出判据。
...
| 字段                        | 是什么                    | 单位      | 判据                                                  | 和谁一起看                                   |
| ------------------------- | ---------------------- | ------- | --------------------------------------------------- | --------------------------------------- |
| `cubic` / `bbr`           | 当前拥塞控制算法               | —       | 它应当与 `sysctl net.ipv4.tcp_congestion_control` 的取值一致 | 是否有 `fq` 队列                             |
| `rtt:x/y`                 | 平滑 RTT / 波动            | 毫秒      | 若它与 `minrtt` 差距大，则路径上有排队                            | `minrtt`                                |
| `minrtt`                  | 观测到的最小 RTT，**路径基线**    | 毫秒      | 它的稳定值就是这条路径的下限                                      | `rtt`、`retrans`                         |
| `cwnd`                    | 拥塞窗口                   | 以 MSS 计 | 若 `cwnd` 小、`minrtt` 大，则窗口受限                         | `snd_wnd`、BDP                           |
| `snd_wnd`                 | 对端通告的接收窗口              | 字节      | 若很小甚至为 0，则对端应用不读数据                                  | 对端的 `Recv-Q`                            |
| `mss` / `pmtu` / `advmss` | 协商 MSS / 路径 MTU / 本端通告 | 字节      | 若 `pmtu` 异常小，则去查路径上的隧道                              | `ip link show`                          |
| `retrans` / `lost`        | 重传段数 / 判定丢失的段数         | 个数      | 出现即说明有丢包；结合重传率看量级                                   | `nstat` 的 `TcpRetransSegs`/`TcpOutSegs` |
| `pacing_rate` / `send`    | 当前 pacing 速率 / 估算发送速率  | 比特每秒    | 把它与链路带宽比率对照                                         | 业务吞吐                                    |
...
| 你看到 | 含义 | 单位与判据 | 和谁一起看 | 转向哪一步 |
| --- | --- | --- | --- | --- |
| 大量 `TIME-WAIT` | 这是主动关闭方的正常状态 | 条数；上限约为出向 QPS × 60 秒 | `ip_local_port_range` 的端口数 | 若远小于端口数则不动手；若接近，则改应用 |
| `TcpExtTCPTimeWaitOverflow` 增量非 0 | 撞上 `tcp_max_tw_buckets`，内核跳过了 `TIME_WAIT` | 次数 | `sysctl net.ipv4.tcp_max_tw_buckets` | 优先减少主动关闭；这个计数是唯一的信号 |
| `CLOSE-WAIT` 堆积 | 应用收到 FIN 没有 `close()` | 条数 | `/proc/<pid>/fd` 的句柄数增长 | 改应用，并补上监控 |
| `FIN-WAIT2` 堆积 | 对端迟迟不发 FIN | 条数 | `tcp_fin_timeout` | 去看对端应用的行为 |
...
| `dmesg` 出现 `table full, dropping packet` | 它说明已经在丢包 | 次数 | `count/max` | 与上一行相同：先治本，再谈调大表 |
...
| 转发了几十 GiB 但表里一条都没有 | 没有规则显式引用 conntrack，内核未注册钩子 | — | `nft list ruleset` 是否为空 | 只有加上 NAT 或 `ct state` 规则，内核才会开始统计条目 |
...
| 你看到 | 含义 | 单位与判据 | 和谁一起看 | 转向哪一步 |
| --- | --- | --- | --- | --- |
| `ping -M do -s 1472` 100% 丢包 | 路径承载不了 1500 字节 | 字节；`-s` 是载荷 | 逐步缩小 `-s` 找出可用 MTU | 去查隧道与 overlay 的封装开销，以及两端的 MTU |
| `ip link show` 的 `mtu` 两端不同 | 配置不一致 | 字节 | `ss -ti` 的 `pmtu`/`mss` | 统一 MTU 或做 MSS clamping |
| `tracepath` 的 `pmtu` 偏小 | 路径上某一跳更小 | 字节 | `mtr -n -r` | 据此定位是那一跳 |
| `ethtool -S` 的 `rx_no_buffer_count` 增长 | 环缓冲不够用 | 次数 | `ethtool -g` 的 current 与 maximum | 排窗口后再调大，因为这项操作会重置网卡 |
| `/proc/net/softnet_stat` 第 2 列增长 | 收包软中断来不及 | 次数（每 CPU 一行） | `net.core.netdev_max_backlog` | 这一项治的是软中断队列，与网卡侧丢包无关 |
| `/proc/net/bonding/*` 的 `Partner Mac Address` 全 0 | LACP 协商未成功 | — | 成员口的 `Aggregator ID` | 两端都要配 LACP，因为只有一端配等于没有聚合 |

`netem` 注入的边界：**队列规则只作用于出口（egress）**。想模拟「对端回包慢」要在对端注入，双向不对称延迟要两端分别注入；在 `lo` 上注入会影响本机所有走回环的流量（含 DNS 存根）。

## 六、五个跨阶段的核对对象

前面按时间顺序讲的是一次请求的旅程；下面五个核对对象各自贯穿其中多个阶段，排障时按它们来切分更快。每一条都点名它贯穿哪几个阶段、每一处读什么字段。

### 6.1 队列与上限：把「够不够」与「谁在排队」分开
...
| 半连接队列 | 由 `listen()` 的 backlog 推导 | `TcpExtTCPReqQFull*` | 内核改用 cookie 完成握手，或者直接丢弃 SYN |
...
### 6.2 证据与计数：为什么必须两个维度对照

| 维度 | 只看它会给出的错误结论 | 正确的用法 |
| --- | --- | --- |
| `ss` 的瞬时状态 | 「队列是空的，所以没溢出」 | 一次采样看不到偶发事件，因此必须与计数器的增量对照 |
...
| 抓包 | 「抓包没有看到就是没问题」 | 它只说明这一跳上没有捕获到；要用两侧对照做二分定位 |

两条通用纪律：**计数器一律取差值**；**任何单一计数器都不能单独定性，必须交叉验证**。

### 6.3 主机内外：什么时候可以判定「问题不在本机」

| 判据 | 说明 | 结论方向 |
| --- | --- | --- |
| 本机 `nstat`/`ss`/`ethtool -S` 全干净 | 它说明本机没有丢包、没有溢出、也没有重传异常 | 因此先怀疑对端或中间设备 |
| 本机发出去了、对端没收到 | 它由两侧同时限时限流的 `tcpdump` 对照得出 | 因此问题在中间链路或过滤设备上 |
| 本机收到但没回 | 它指向 `INPUT` 链、`rp_filter` 与 conntrack | 因此去查本机的过滤与连接跟踪 |
| 主机侧干净且路径经过安全组、LB 或云网络 | 这些设备都在主机之外 | 因此去找云平台侧确认 |

**这一节的实用价值是省时间**：主机侧计数干净时，继续在本机调参数不会让问题消失。

### 6.4 变更纪律：任何一次改动都要能回滚
...
| 看到 `Recv-Q` 是 0 就排除队列问题 | 一次采样看不到偶发事件，因此要看计数器的增量 |
| 「没有重传」用于排除 UDP 丢包 | UDP 没有这两类计数器，必须靠应用指标或抓包 |
...
| 「`si`/`so` 非 0 就是内存不够」 | 这两个数衡量的是换入换出速率，属于内存压力而不是网络问题；本章只用它排除网络侧成因，判断内存要看 `/proc/pressure/memory` 的 `some`/`full` 与 `/proc/vmstat` 的 `pgmajfault` |
| 「主机侧计数干净，多调几个参数试试」 | 主机外的问题不会因为本机调参而消失，还会破坏可回滚的状态 |
| 「抓包没看到包就说明这段没问题」 | 它只能说明这一跳上没有捕获到；要对照抓包位置与过滤条件才能下结论 |
