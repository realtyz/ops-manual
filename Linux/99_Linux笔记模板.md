...
> 1. 复制本文件，重命名为 `NN_主题.md`，`NN` 与 [[Linux/00_简介|00_简介]] 里的阶段编号保持一致。
> 2. 在 [[Linux/00_简介|00_简介]] 对应阶段下补上「对应笔记」链接和 checklist 条目。
> 3. 按下面的骨架填写。`<!-- -->` 里的提示只在编辑视图可见，写完可以整段删掉。
> 4. 定稿前对照文末的「自检清单」逐条过一遍。

> 本笔记对应 [[Linux/00_简介|Linux 大纲]] 的「阶段 N」。目标：〈用一句话说清学完能讲明白什么〉。
>
> 关联复习：[[Linux/00_简介|00_简介]]
>
...
- [ ] [[Linux/00_简介|00_简介]] 里已补上对应阶段的 checklist 与链接
