...
> 本文结论来自内核源码、man 页与发行版文档，**尚未在实验机上逐条复现**：凡是「默认值」都给查本机的命令。
> 建议顺序：先取基线（`free -h`、`cat /proc/meminfo`、`sysctl vm.swappiness vm.overcommit_memory`、`stat -fc %T /sys/fs/cgroup`），再按第 4 节的实验逐个做，把实测输出回填，最后核对文末「自检清单」。
...
> 如果只做一个实验，选第 4 节的第 2 个（tmpfs 占内存却不出现在 `used` 里）——它一次性讲清了「内存去哪了」的核心误区。
...
| 大页 | 显式 HugePages 与透明大页 THP，用 2MB/1GB 页代替 4KB 页以减少 TLB 与页表开销 | 收益是 TLB 命中与吞吐；代价是**内存碎片与偶发卡顿**（THP 的 `always` + 后台整理），见 1.7 |
...
numastat -m                          # 验证：按 NUMA 节点的内存分布（见 1.8）
```

### 1.2 `free` 与 `/proc/meminfo` 的正确读法
...
| 脏的 file 页 | 先回写到磁盘再丢弃 | 磁盘（见 1.3 的回写参数） |
...
> **「有 swap 就是慢」**：不对。排在 LRU 末尾、长期不用的页被换出去，反而给热数据腾了内存；只有**频繁换入换出（thrashing）**才意味着灾难，判据是 `vmstat` 的 `si`/`so` 长期非 0，以及 PSI 的 `memory some/full` 上升。
...
- 只有**同一轮 OOM 事件内的候选进程**之间比较得分，跨 cgroup 不比较（cgroup OOM 只在该 cgroup 内选，见 1.6）。
...
分配不出来的那一刻，内核做的事情按下面这条链走——**这也是「延迟抖动」与「进程被杀」的分岔图**：

```mermaid
flowchart TD
  A["进程申请内存<br/>malloc 或缺页触发"] --> B{"空闲页高于 min 水位吗"}
  B -- "是" --> C["分配成功"]
  B -- "否" --> D["唤醒 kswapd<br/>或由分配者 direct reclaim"]
  D --> E{"能回收出页吗"}
...
| `memory.peak` | 历史峰值（较新内核） | 事后复盘「当时最高到多少」非常有用 |
...
### 1.7 内存核算：VSZ/RSS/PSS 与「内存去哪了」

先把四个口径分清楚，否则「这个进程占多少内存」这句话本身就没有答案：
...
接下来是这道题的核心：**「内存去哪了」只有三类答案**，而这三类在 `free` 里都不显眼。
...
```text
journalctl -k -b | grep -i -E 'out of memory|oom-kill|killed process'   # 验证：OOM 事件与上下文
journalctl -k -b | grep -B30 'Killed process' | head -60               # 验证：连水位块一起看
cat /sys/fs/cgroup/.../memory.events                                   # 验证：cgroup 侧的 oom_kill 计数
...
1. **找到 OOM 日志**：`journalctl -k` 里搜 `Killed process`，看前缀区分整机 OOM 与 cgroup OOM。
2. **看被选中进程的得分依据**：`anon-rss`、`pgtables`、`oom_score_adj`。
3. **看当时水位**：日志里 zone 的 `free/min/low/high` 块；`min` 附近徘徊说明一直在水位边缘。
4. **对齐 cgroup 侧证据**：`memory.events` 的 `oom_kill`、`memory.current`/`memory.peak`、`memory.stat` 的构成。
5. **确认限制来源**：`memory.max` 太小，还是整机真的没内存，还是某个缓存/临时文件突然涨上来。

```text
journalctl -k -b | grep -i -E 'Memory cgroup out of memory|Out of memory: Killed process'
journalctl -k -b | grep -B25 'Killed process' | head -50      # 验证：连同 zone 水位一起看
...
> [!warning] 前置纪律
> 实验 3 会改内核回写参数、实验 4 会创建 swap 并故意打压力、实验 5 会触发 cgroup OOM，**只在可快照的实验机上做**，并把改过的参数逐条恢复。生产机上只做只读观测。

> [!example]- 实验 1：Page Cache 是资产，且「可回收」
> ```text
...
> [!example]- 实验 2：tmpfs 占的是内存，而 `used` 常常看不见它
> ```text
...
> [!example]- 实验 4：swap 与 swappiness，以及 PSI 怎么读
> ```text
...
> [!example]- 实验 5：触发一次 cgroup OOM，并按六步取证
> ```text
...
> [!example]- 实验 6：RSS 会重复计数，PSS/USS 才是「真占用」
> ```text
