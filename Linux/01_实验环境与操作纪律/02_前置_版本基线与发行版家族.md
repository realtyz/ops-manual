...
>
> 实测状态：四项基线与资源、时间、启动参数输出已在 **VMware 虚拟机 Ubuntu 24.04.5 LTS** 上实测并粘贴（本次迁移重跑，替换了原 WSL2 输出）。RHEL/SUSE/Alpine 的输出未实测，其生命周期与默认栈以官方文档为准。**发行版的小版本号与 EOL 变化很快，本文只给「节奏与血统」层面的量级，具体数字一律现场查。**
...
>   怎么验证：`cat /etc/os-release`、`uname -r`、`systemctl --version | head -1`、`stat -fc %T /sys/fs/cgroup`；实测本机依次是 `PRETTY_NAME="Ubuntu 24.04.5 LTS"`、`6.8.0-139-generic`、`systemd 255 (255.4-1ubuntu8.17)`、`cgroup2fs`。
...
实测输出（同一次实测，磁盘用量与 `MemTotal` 随开机略有浮动）：
...
/dev/mapper/ubuntu--vg-ubuntu--lv ext4   47G  8.5G   37G  20% /
           ...
           Universal time: Sat 2026-09-19 08:03:08 UTC
                 RTC time: Sat 2026-09-19 08:03:08 UTC
BOOT_IMAGE=/vmlinuz-6.8.0-139-generic root=/dev/mapper/ubuntu--vg-ubuntu--lv ro
```

> [!info] 这三组输出里，有四处和「典型教程」不一样
> - **根分区在 LVM 上**，不是 `/dev/sda3` 这种裸分区：`/dev/mapper/ubuntu--vg-ubuntu--lv`，容量 47 G（`df -hT` 的 GiB 口径）。任何「先看 `/dev/sdX`」的教程在这里都要先 `lsblk` 找一层 `LVM2_member`。
> - **内核没有平台后缀**：`6.8.0-139-generic`。这行本身就是「内核由本机虚拟化平台提供、还是发行版官方内核」的判据——WSL2 会带 `microsoft-standard-WSL2` 后缀。
> - **时区是 `Etc/UTC`**，不是 `Asia/Shanghai`：写排障记录时，日志时间与你的钟表可能差 8 小时。
> - **`/proc/cmdline` 只有一行、没有 `WSL_*` 变量**：本机是 legacy BIOS + GRUB 引导的普通虚拟机，启动参数由 `/etc/default/grub` 决定。
...
| 项目 | 命令 | 实测示例（本文环境） |
| --- | --- | --- |
| 发行版 | `cat /etc/os-release` | Ubuntu 24.04.5 LTS（noble） |
| 内核 | `uname -r` | 6.8.0-139-generic |
| 启动参数 | `cat /proc/cmdline` | `/vmlinuz-6.8.0-139-generic root=/dev/mapper/ubuntu--vg-ubuntu--lv ro` |
| systemd | `systemctl --version` | systemd 255（255.4-1ubuntu8.17） |
| cgroup | `stat -fc %T /sys/fs/cgroup` | `cgroup2fs`（v2） |
| 资源 | `nproc`、`grep MemTotal /proc/meminfo` | 4 vCPU / 8083496 kB（7894 MiB） |
| 根文件系统 | `df -hT /`、`df -i /` | `/dev/mapper/ubuntu--vg-ubuntu--lv` ext4 47G（用 20%，inode 3145728 用 6%） |
| 时间 | `timedatectl` | `Etc/UTC (UTC, +0000)`，`System clock synchronized: no` |
| 虚拟化与启动模式 | `systemd-detect-virt`、`ls /sys/firmware/efi` | `vmware` / 无 EFI 目录（**legacy BIOS**） |
| 工具版本 | `iostat -V`、`ss -V`（**不是 `sysstat -V`**：本机没有这个命令，会报 `bash: sysstat: command not found`） | `sysstat version 12.6.1`、`iproute2-6.1.0`；记下来，字段名随版本变 |
...
- **虚拟化平台的内核版本不代表物理机**：WSL2 的内核带 `microsoft-standard-WSL2` 后缀，说明是平台定制内核；**本机是 VMware 全虚拟化，`uname -r` = `6.8.0-139-generic`，是发行版官方内核，没有平台后缀**。判断方法就是看后缀：有 `-microsoft-standard-WSL2`、`-aws`、`-azure` 这类后缀的，行为可能与通用内核不同。`uname -r` 一样要记，但别拿它去推断物理服务器的行为。
- **启动模式要现场测，不能猜**：`ls /sys/firmware/efi` 无输出 = **legacy BIOS**（本机实测就是这种）。这一条决定 `efibootmgr`、Secure Boot、MOK 一类操作在本机成不成立。
...
- **建议至少两台**：一台 Ubuntu LTS（文档多、生态全，作为主实验机）+ 一台 Rocky/AlmaLinux（练 `dnf`、`firewalld`、SELinux、XFS）。**本目录的实测环境是一台 Ubuntu 24.04.5 LTS 的 VMware 虚拟机（4 vCPU / 7894 MiB / LVM 根分区 / legacy BIOS 启动）**，涉及 RHEL 系的部分以官方文档为准。
- **用虚拟机而不是 WSL2 当主实验机**：本机 `systemd-detect-virt` 报 `vmware`、有独立内核与真实块设备（`lsblk` 能看到 `sda`/`sr0`），所以 loop 挂载、临时文件系统、swap、持久 journal 这些都能真跑；WSL2 的内核由平台提供，这几类做不了。
...
| 把 `uname -r` 的输出当物理机行为 | WSL2、云主机、容器的内核都由平台提供（如 `6.6.87.2-microsoft-standard-WSL2`），行为与物理机可能不同；**看后缀就能分辨**：本机是 VMware 全虚拟化，`6.8.0-139-generic` 是发行版官方内核 |
| 不确认启动模式就照抄 GRUB/UEFI 教程 | 本机 `/sys/firmware/efi` 不存在（legacy BIOS），`efibootmgr`、Secure Boot、MOK 一类操作在这里不成立 |
