...
>
> 关联复习：[[03_进程与信号]]（D 状态为什么杀不掉、自愿/非自愿切换、句柄上限）、[[04_内存管理与OOM]]（`available` 口径、Page Cache、OOM 判定、PSI）、[[05_存储与文件系统]]（`iostat` 各列、`df`/`du`、NFS 卡死）、[[06_网络协议栈与排障]]（TCP 状态机、`ss -ti`、`nstat`、抓包）、[[08_systemd与服务管理]]（cgroup 与 unit 资源限制、`systemd-cgtop`）、[[09_日志与监控]]（三条链路、USE 方法、分位数与告警）、[[11_生产运维与高可用]]（变更窗口与回滚）、[[12_复习与自测]]，以及 AI 基础设施主题域的 [[AI-Infra/07_XID与硬件故障处理|XID 与硬件故障]]、[[AI-Infra/06_NCCL排障|NCCL 排障]]（GPU/高速网卡的下钻方法不在本文展开）。
>
...
| **GPU/高速网卡** | XID、掉卡、NCCL 通信静默降速 | `nvidia-smi dmon`、`rdma link`/`ibv_devinfo`、`ethtool -S` 的 pause/errors | 见 [[AI-Infra/07_XID与硬件故障处理|XID 与硬件故障]]、[[AI-Infra/06_NCCL排障|NCCL 排障]] |
