...
> 本笔记对应 [[00_学习大纲|Linux 大纲]] 的「阶段 10：性能分析与故障排查 ★（核心区）」。目标：把「CPU/内存/IO/网络/锁」五层资源讲成**一套可复用的排除法**——先量化影响面与时间点，再用工具取同时刻的证据，逐层排除后给出根因、止损与长期改进；并练到「10 分钟内把一个故障定位到具体资源与具体进程」。
>
> 关联复习：[[Linux/03_进程与信号/00_导读与知识地图|03 进程与信号]]（D 状态为什么杀不掉、自愿/非自愿切换、句柄上限）、[[Linux/04_内存管理与OOM/00_导读与知识地图|04 内存管理与 OOM]]（`available` 口径、Page Cache、OOM 判定、PSI）、[[Linux/05_存储与文件系统/00_导读与知识地图|05 存储与文件系统]]（`iostat` 各列、`df`/`du`、NFS 卡死）、[[Linux/06_网络协议栈与排障/00_导读与知识地图|06 网络协议栈与排障]]（TCP 状态机、`ss -ti`、`nstat`、抓包）、[[08_systemd与服务管理]]（cgroup 与 unit 资源限制、`systemd-cgtop`）、[[09_日志与监控]]（三条链路、USE 方法、分位数与告警）、[[11_生产运维与高可用]]（变更窗口与回滚）、[[12_复习与自测]]，以及 AI 基础设施主题域的 [[AI-Infra/09_XID与硬件故障处理|XID 与硬件故障]]、[[AI-Infra/06_NCCL排障|NCCL 排障]]（GPU/高速网卡的下钻方法不在本文展开）。
>
...
- **单一指标会骗人**：负载高 ≠ CPU 忙（含 D 状态）；`%util` 高 ≠ 一定排队（虚拟设备可能并行）；平均值掩盖长尾；`free` 少 ≠ 内存不足（[[Linux/04_内存管理与OOM/00_导读与知识地图|04 内存管理与 OOM]]）。
...
| `si` 高 | 软中断（多为网络） | `mpstat` 的 `%soft`、`/proc/net/softnet_stat` | 网卡多队列/RPS、`ethtool -S`（[[Linux/06_网络协议栈与排障/00_导读与知识地图|06 网络协议栈与排障]]） |
...
判读要点：**中断是否集中在少数 CPU**（单队列网卡 → 一个核被 `si` 打满）、`softnet_stat` 第 2 列是否增长（收包来不及处理会丢）、以及虚拟化环境里中断由谁投递（本机是 Hyper-V 的 MSI）。多队列（RSS）与软件分流（RPS/RFS）的细节在 [[Linux/06_网络协议栈与排障/00_导读与知识地图|06 网络协议栈与排障]]。
...
| `MemAvailable` | 应用真正还能申请到的量（含可回收缓存） | 告警应该用它（[[Linux/04_内存管理与OOM/00_导读与知识地图\|04 内存管理与 OOM]]） |
...
- **确认泄漏要看趋势不看瞬时值**：按天/周取样 RSS/PSS，区分「真泄漏」「缓存增长」「业务自然增长」（[[Linux/04_内存管理与OOM/00_导读与知识地图|04 内存管理与 OOM]]）。
...
**`fsync` 的 p50 是纯缓冲写的约 4200 倍**。所以：日志用「缓冲 + 定期刷盘」或异步追加；确需持久化的关键点（事务提交）才 `fsync`；把日志盘与数据盘分开（[[Linux/05_存储与文件系统/00_导读与知识地图|05 存储与文件系统]]）。顺序读的 Page Cache 效应也顺手测了：同 512MiB 文件 `iflag=direct` 读 3.5GB/s，普通读 15.8GB/s。

### 1.6 网络：先看计数器，再决定抓不抓包【核心】

网络排障的入口和判据在 [[Linux/06_网络协议栈与排障/00_导读与知识地图|06 网络协议栈与排障]]，这里只保留「性能视角」的三件事：
...
**CPU throttling 是容器里最常见、也最容易被误判的「性能问题」**：容器内 CPU 使用率不高、但延迟周期性抖动，宿主侧 `cpu.stat` 的 `nr_throttled` 在涨——这是配额被周期性用满后强制限流，不是应用本身慢。同理，`memory.events` 的 `oom_kill` 增长说明被杀过（[[Linux/04_内存管理与OOM/00_导读与知识地图|04 内存管理与 OOM]]、[[Kubernetes/03_调度_资源_QoS|Kubernetes 资源与 QoS]]）。
...
| **D 状态堆积（IO/挂载卡死）** | load 高、CPU 空闲、进程杀不掉、`vmstat` 的 `b` > 0 | `ps -eo stat` 找 D → `wchan` → `mount`/NFS/后端盘 | 恢复后端/摘挂载；[[Linux/05_存储与文件系统/00_导读与知识地图|05 存储与文件系统]]、[[Linux/03_进程与信号/00_导读与知识地图\|03 进程与信号]] |
| **direct reclaim / THP 抖动** | 延迟周期性尖刺、`%sy` 上升、`zone_reclaim` 类计数器变化 | PSI memory some/full、`/proc/vmstat` 的 `pgscan_direct`、THP 设置 | 降内存水位/调 THP 策略/加内存（[[Linux/04_内存管理与OOM/00_导读与知识地图\|04 内存管理与 OOM]]） |
| **DNS 解析慢** | 请求耗时分布出现固定台阶、日志里连接前的等待 | `getent hosts` 计时、`ss -ti` 无异常、`resolvectl`/`dig` 对比 | 缓存/换解析链路；[[Linux/06_网络协议栈与排障/00_导读与知识地图|06 网络协议栈与排障]] |
...
| **端口耗尽（TIME_WAIT）** | 「偶发连接失败」、`ss -s` 的 timewait 暴涨 | 实测：端口范围 32768–60999（**28232 个**），3000 条连接 0.1s 后 TIME_WAIT 3023 个 | 长连接/连接池、扩端口范围、`tcp_tw_reuse`（[[Linux/06_网络协议栈与排障/00_导读与知识地图|06 网络协议栈与排障]]） |
| **句柄耗尽（EMFILE）** | 「Too many open files」、偶发失败 | `ulimit -n`、`/proc/<pid>/fd` 数量、`lsof -p` | 修泄漏/调 `LimitNOFILE`（[[Linux/03_进程与信号/00_导读与知识地图\|03 进程与信号]]）；实测 ulimit 10240 时第 10236 个 fd 报 `Errno 24` |
...
> - 不要用 `kill -9` 处理 D 状态进程——杀不掉，还会丢掉应用清理锁/临时文件的机会（[[Linux/03_进程与信号/00_导读与知识地图|03 进程与信号]]）。
...
mount | grep -E "nfs|cifs|fuse"   # 网络存储卡死是常见根因（[[Linux/05_存储与文件系统/00_导读与知识地图|05 存储与文件系统]]）
```
...
| `si` | 多队列/RPS、软中断分摊、网卡 offload（[[Linux/06_网络协议栈与排障/00_导读与知识地图|06 网络协议栈与排障]]） |
| `wa` | IO 层优化；注意 `wa` 高但 `iostat` 干净时要查 NFS/虚拟化（[[Linux/05_存储与文件系统/00_导读与知识地图|05 存储与文件系统]]） |
...
顺序：**确认是不是真的不够（available + PSI）→ 是不是被杀过（dmesg/cgroup `memory.events`）→ 谁占的（RSS/PSS）→ 是泄漏还是缓存/GC（趋势）**。细节与 OOM 评分见 [[Linux/04_内存管理与OOM/00_导读与知识地图|04 内存管理与 OOM]]。
...
smartctl -a /dev/sdX | grep -E "Reallocated|Pending|Percentage_Used"   # 健康（[[Linux/05_存储与文件系统/00_导读与知识地图|05 存储与文件系统]]）
```
...
tcpdump -i <dev> -c 200 -w /tmp/cap.pcap "host X and port Y"   # 限时限量（[[Linux/06_网络协议栈与排障/00_导读与知识地图|06 网络协议栈与排障]]）
```
...
**处置纪律**：D 状态的进程 `kill -9` 无效（信号要等它回到用户态才处理），先恢复它等待的资源（NFS 服务端、块设备、锁），必要时 `umount -f`/重启整机（[[Linux/03_进程与信号/00_导读与知识地图|03 进程与信号]]、[[Linux/05_存储与文件系统/00_导读与知识地图|05 存储与文件系统]]）。
...
实测模型：端口范围 32768–60999 共 **28232** 个；3000 条短连接在 0.1 秒内制造了 **3023 个 TIME_WAIT**（每个默认停留 60 秒），**因此单机对「同一目标端口」的短连接上限约为 28232/60 ≈ 470 条/秒**。句柄侧实测：`ulimit -n=10240` 时第 10236 个 fd 报 `Errno 24 Too many open files`。两者的表现都是「偶发失败」，不是「持续失败」（[[Linux/03_进程与信号/00_导读与知识地图|03 进程与信号]]、[[Linux/06_网络协议栈与排障/00_导读与知识地图|06 网络协议栈与排障]]）。
...
> - **`free` 的「空余」口径**：要看 `MemAvailable` 而不是 `MemFree`，还要看 PSI；`vmstat` 的 `si/so` 长期非 0 说明确实在换页（[[Linux/04_内存管理与OOM/00_导读与知识地图|04 内存管理与 OOM]]）。
...
> - **块层**：`blktrace`/`biosnoop` 关联到具体块与进程（需 root/eBPF）；`smartctl` 排除介质健康（[[Linux/05_存储与文件系统/00_导读与知识地图|05 存储与文件系统]]）。
...
> - `si`：`/proc/net/softnet_stat` + `ethtool -S` + 多队列/RPS（[[Linux/06_网络协议栈与排障/00_导读与知识地图|06 网络协议栈与排障]]）。
...
- [x] 跨笔记引用都用 wikilink；跨目录引用用路径写法（[[Kubernetes/03_调度_资源_QoS|Kubernetes 资源与 QoS]]、[[AI-Infra/06_NCCL排障|NCCL 排障]]）；指向尚未创建的阶段笔记（[[11_生产运维与高可用]]）的链接与 [[00_学习大纲|00_学习大纲]] 的规划一致
- [x] [[00_学习大纲|00_学习大纲]] 的阶段 10 已有「对应笔记」链接与 checklist 条目，本次补齐了观测开销与采样偏差、一次性取证、六段报告模板、容器/宿主双视角、工具降级路径、端口/句柄量化模型等条目
