...
>
> **实测环境：Ubuntu 24.04.5 LTS（VMware 虚拟机）/ 内核 `6.8.0-139-generic` / systemd 255（`255.4-1ubuntu8.17`）/ root 可用。** 应用日志的落点行为在本机用 `logger`、`systemd-cat` 与 `systemd-run` 起的临时单元验证（实验后已删除，`systemctl list-unit-files 'lab09a*'` 为空）；`journald` 持久化在 `/var/log/journal`，`rsyslog` 已安装并运行，`ForwardToSyslog=yes` 生效，所以「stdout / 自有文件 / syslog 套接字」三种落点都能在本机看到真实差异。
...
>   - 证据：实测 `systemd-run --unit=lab09a-speclog --property=StandardOutput=journal /bin/echo '{"level":"info","msg":"hi"}'`，进 journald 后 `MESSAGE` 里是**原文**，而 `PRIORITY=6`、`_SYSTEMD_UNIT=lab09a-speclog.service`、`_TRANSPORT=stdout` 是 journald 自己加的。
> - **一条可排障的日志最低要带：时间、级别、服务/实例、请求 ID、可检索的错误码。**
>   - 怎么验证：随手拿一条线上日志对照这五项；缺哪项，就在对应的排障场景里查不出东西。
> - **JSON 行是最省事的折中。**
>   - 证据：实测 `journalctl -u lab09a-speclog -o cat` 输出的就是写进去的那行 `{"level":"info","msg":"hi"}`，人可读、机器也能按字段解析。
> - **日志不该承担「精确聚合」的职责。**
...
> ```bash
> systemd-run --unit=lab09a-speclog --property=StandardOutput=journal \
>   /bin/echo "{\"level\":\"info\",\"msg\":\"hi\"}"   # 验证：把一行 JSON 写进 journald（root 直接起系统级临时单元）
> journalctl -u lab09a-speclog -o cat | grep level   # 验证：消息内容原样落在 journald
> journalctl -u lab09a-speclog -o json | python3 -c "
> import json,sys
> for l in sys.stdin:
>     d=json.loads(l)
>     if d.get('MESSAGE','').startswith('{'):
>         print({k:d[k] for k in ['_SYSTEMD_UNIT','_TRANSPORT','PRIORITY','MESSAGE','_PID'] if k in d})
> "
> grep 'lab09a-speclog\|{\"level\"' /var/log/syslog | tail -1   # 验证：同一行也落到了 rsyslog 侧
> systemctl stop lab09a-speclog.service; systemctl reset-failed lab09a-speclog.service   # 清理
> systemctl list-unit-files 'lab09a*'                  # 验证：没有残留单元（应为 0 unit files listed）
> ```
> 本机实测输出：
> ```text
> Running as unit: lab09a-speclog.service
> {"level":"info","msg":"hi"}
> {'_SYSTEMD_UNIT': 'lab09a-speclog.service', 'SYSLOG_IDENTIFIER': 'echo', 'PRIORITY': '6', '_TRANSPORT': 'stdout', 'MESSAGE': '{"level":"info","msg":"hi"}', '_PID': '24916'}
> ```
> 要注意三点：① `MESSAGE` 里是**你写的那一行 JSON 原文**（journald 不会替你解析成字段），而 `PRIORITY=6`、`_SYSTEMD_UNIT`、`_TRANSPORT=stdout` 是 journald **自己加上的**字段；② `_TRANSPORT=stdout` 正是「这个服务把日志写在 stdout」的直接证据——对比 `logger` 造的消息是 `_TRANSPORT=syslog`，两者在 journald 里一眼可分辨；③ 想让 JSON 里的字段可查询，需要采集端或 journald 的解析能力配合（子笔记 05）。
> **风险**：低（只创建一个临时单元，不动任何系统服务，不重启 `systemd-journald`）。
> **回滚**：`systemctl stop lab09a-speclog.service; systemctl reset-failed lab09a-speclog.service`。
> **耗时**：10 分钟。
>
> 环境：Ubuntu 24.04.5（VMware 虚拟机）/ 内核 6.8.0-139-generic / systemd 255 / root。本次实测 `--property=StandardOutput=journal` 生效，`PRIORITY=6` 对应 info。**不需要再手工 `export XDG_RUNTIME_DIR`**：本机走的是系统级单元，`systemd-run` 直接可用；即使是 `realtyz`（uid=1000）用户级会话，`/run/user/1000` 由 `systemd-logind` 自动创建、`XDG_RUNTIME_DIR` 也已自动注入（`systemctl --user is-system-running` 返回 `running`），不会再出现「非 root 环境里 `Failed to connect to bus`」那种问题。生产机上只做「读」的部分（`journalctl`、`systemctl show -p StandardOutput`）。
