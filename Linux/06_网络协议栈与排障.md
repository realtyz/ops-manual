...
> 1. **一台能 root 的 Linux 实验机**（虚拟机或 WSL2 都行），装了 `iproute2` 与 `python3`。第 4 节要在隔离的 network namespace 里建 veth 拓扑，**容器里通常做不了**（需要 `CAP_NET_ADMIN`，实验 3 伪造 SYN 还需要 `CAP_NET_RAW`）。
> 2. **知道实验要按顺序做**：实验 3 建好的 `srv`/`cli` 拓扑会被实验 5、6、7 复用，所以**先把 1～7 全部做完，最后统一清理**；顺序与清理命令见第 4 节开头的「实验环境与顺序」。
> 3. **读过相邻三篇**：D 状态与句柄上限（[[03_进程与信号]]）、缓冲区换吞吐的内存代价（[[04_内存管理与OOM]]）、`_netdev` 与网络存储挂载点卡死（[[05_存储与文件系统]]）。
>
> 环境提醒：本文的实测数字来自 **WSL2 上的 Ubuntu 24.04 / 内核 6.6 + 虚拟网卡**，物理网卡相关的字段（`ethtool -S/-g/-l`、`/proc/interrupts` 的队列分布）与真实跨网链路的数字不适用，详见文末「验证进度」。
...
> - **「客户端偶发连不上、服务端啥日志都没有」的头号嫌疑是全连接队列溢出**：上限是 `min(listen() 的 backlog, somaxconn)`——实测 `listen(10000)` 在 `somaxconn=4096` 的机器上被静默截断 **4096**，所以「只调大应用 backlog」经常是无效动作；溢出时内核默认**静默丢弃** SYN（`tcp_abort_on_overflow=0`），客户端卡在 `SYN-SENT` 重传，服务端只在 `nstat` 的 `TcpExtListenOverflows`/`ListenDrops` 里留下痕迹——**实测就是这个形状**。
...
> [!tip] 怎么用这篇笔记：分三遍读
> 全篇按难度分了两层：标 **【核心】** 的属于阶段 6 必须掌握；标 **【进阶】** 的只需要知道有这条路，用到时再回来查。
> - **第一遍（约半天，先把「连不上」处理掉）**：第 0 节 → 1.1 分层地图 → 1.2 握手与两个队列 → 1.3 挥手与两个状态 → 3.1 连不上 → 3.4 队列溢出 → 实验 1、2。这一遍结束应该能独立处理「偶发超时、服务端没日志」。
> - **第二遍（约一天，把链路、参数与解析补齐）**：1.4 关键内核参数 → 1.5 拥塞控制与 `ss -ti` → 1.6 路由与 conntrack → 1.8 DNS 解析链 → 2.1 基线 → 3.2、3.3、3.5、3.6、3.8 → 实验 3、4。
> - **第三遍（进阶与演练）**：1.7 链路层与 MTU → 1.9 抓包与 netem → 1.10 主机侧调优 → 2.2、2.3 → 3.7、3.9、3.10 → 实验 5、6、7（复用实验 3 的拓扑，**做完再统一清理**）。
> 面试前只看第 0 节与第 5 节的问答卡。如果只做两个实验，选**实验 1**（全连接队列溢出）与**实验 3**（syncookies 对照）——它们是这篇最值钱、也最容易在生产上遇到的现场。
...
> | 名词 | 一句话解释 | 在哪一节展开 |
> | --- | --- | --- |
> | 半连接队列 / 全连接队列 | 收到 SYN 后的 `SYN_RECV` 队列 / 握手完成、等 `accept()` 的队列 | 1.2 |
> | `Recv-Q` / `Send-Q`（LISTEN） | 监听套接字的当前队列深度 / 队列上限 | 1.2 |
> | `backlog` / `somaxconn` | `listen()` 传入的队列长度 / 它被截断时的全局上限 | 1.2、1.4 |
> | syncookies | 半连接队列满时不排队、直接用 cookie 完成握手的兜底机制 | 1.2、1.4 |
> | `TIME_WAIT` / `2MSL` | 主动关闭方等 60 秒的状态 / 等两个最大报文生存期，确保 ACK 送达且旧包消散 | 1.3 |
> | `FIN_WAIT2` | 已发 FIN、等对端 FIN 的状态，由 `tcp_fin_timeout` 控制 | 1.3 |
> | `cwnd` / BDP | 拥塞窗口（按 MSS 计）/ 带宽延迟积（带宽 × RTT），决定单流吞吐上限 | 1.5 |
> | `rtt` / `minrtt` | 平滑 RTT / 观测到的最小 RTT，两者差距大就是有排队 | 1.5 |
> | `app_limited` | 连接没被应用喂满——瓶颈在应用，不在网络 | 1.5 |
> | `pacing_rate` / `delivered` / `lost` | 内核预估发送速率 / 已确认字节 / 判定丢失的字节 | 1.5 |
> | conntrack | 内核连接跟踪表，NAT 与 state 规则的基础；表满会静默丢包 | 1.6 |
> | 策略路由（`ip rule`） | 按源地址、fwmark 等条件选择不同的路由表 | 1.6 |
> | `rp_filter` | 反向路径过滤，非对称路由下会误杀正常流量 | 1.6、2.1 |
> | bond / LACP | 多网卡聚合 / 需要两端协商的模式 4 | 1.7 |
> | PMTU / PMTUD | 路径 MTU / 依赖 ICMP 的发现机制，ICMP 被禁就成黑洞 | 1.7 |
> | MSS clamping | 在网关上改写 SYN 的 MSS，规避 PMTU 黑洞 | 3.7 |
> | NSS（`nsswitch.conf`） | 名称解析的插件顺序，默认 `files` 先于 `dns` | 1.8 |
> | `ndots` / `search` | resolv.conf 的后缀尝试规则，导致一次解析可能查 4~5 次 | 1.8 |
> | `netem` | `tc` 的故障注入模块（delay/loss/reorder/corrupt/rate） | 1.9 |
> | `fq` / `fq_codel` | BBR 需要的 pacing 队列 / 现代发行版的默认队列规则 | 1.5、1.9 |
> | `softnet_stat` | 每个 CPU 的收包统计（第 2 列是 dropped） | 1.10 |
> | RSS / RPS / RFS | 网卡多队列 / 软件收包分流 / 按流分流 | 1.10 |
> | `TcpExt*` 计数器 | `nstat` 暴露的协议栈计数（`ListenOverflows`、`SyncookiesSent`、`TCPReqQFull*`…） | 1.2、1.4 |

### 1.1 先建立地图：一次连接要穿过哪几层【核心】
...
> 对应实验：第 4 节**实验 1**（全连接队列溢出）、**实验 2**（`somaxconn` 截断 `listen()`）、**实验 3**（半连接队列与 syncookies 对照）。

### 1.3 四次挥手：TIME_WAIT 与 CLOSE_WAIT【核心】
...
> 对应实验：第 4 节**实验 4**（实测主动关闭方进入 `TIME_WAIT`，并计时到消失）。

### 1.4 关键内核参数：语义、默认值与副作用【核心】
...
> 对应实验：第 4 节**实验 7**（用 `ss -ti` 与 `ethtool` 拿链路证据，不用抓包）。
...
> 对应实验：第 4 节**实验 6**（在 veth 两端配不对称的 MTU，用 `ping -M do` 复现「小包通、大包丢」）。

### 1.8 DNS 解析链：为什么 `getent` 和 `dig` 结果不一样【核心】
...
> 对应实验：第 4 节**实验 5**（在 veth 上注入 50ms 延迟与 20% 丢包，看 RTT 与丢包率的实测变化）。

### 1.10 延伸：主机侧网络调优的观测入口【进阶】
...
**下面这条路径就是 2.3「分层排查的固定姿势」在「连不上」这个现象上的具体化**（2.3 给顺序与原则，这里给命令与判据）：
...
> [!warning] 前置纪律
> 下面 7 个实验的输出全部是**在实验机上实测粘贴**的（环境见文末「验证进度」）。其中实验 3~6 会改动网络配置，**统一在独立 network namespace 里做**，宿主网络栈不受影响；实验结束后按本节开头的统一清理命令收尾。
> 破坏性/干扰性动作（伪造 SYN、netem 注入、改 MTU）**只在实验机做**，且动手前先确认自己在哪台机器上。

> [!info]- 实验环境与顺序（开跑之前看这里）
> **顺序**：实验 3 建立 `srv`/`cli` 两个 network namespace 与一对 veth，**实验 5、6、7 复用它**；实验 1、2、4 相互独立。按 1 → 2 → 3 → 4 → 5 → 6 → 7 做完再清理，**不要在做完实验 3 的「回滚」之后又去跑 5/6/7**。
> ```text
...
>
> **实验与正文的对应**：实验 1、2、3 → 1.2 三次握手与两个队列；实验 4 → 1.3 挥手与 `TIME_WAIT`；实验 5 → 1.9 抓包与故障注入；实验 6 → 1.7 链路层与 MTU；实验 7 → 1.5 拥塞控制与 `ss -ti`。
> **没有配套实验的小节**：1.6 路由与 conntrack、1.8 DNS 解析链、1.10 主机侧调优——它们依赖真实网络、DNS 环境或物理网卡，本文只给只读的观测命令（物理网卡部分见「验证进度」里未勾选的条目）。
...
```text
# ① 服务端：只 listen(1)，永不 accept()。放后台，保持 10 分钟
python3 - <<'PY' &
import socket, time
s = socket.socket()
...
# ② 客户端：依次建 6 条连接，每条都保持不关（放后台，保持 90 秒）
sleep 1
python3 - <<'PY' &
import socket, time
held = []
...
# ③ 取证：等 10 秒，让前几条连接都试过一轮（第 3 条起会卡在 connect 上重传）
sleep 10
ss -lnt | grep 9999
ss -tan | grep 9999
...
回滚：**先不要删**——实验 5、6、7 还要复用这套 `srv`/`cli` 拓扑；全部做完后执行本节开头的统一清理命令（`ip netns del srv; ip netns del cli` 会连同 veth 一起回收，实测无残留）。
...
```text
# 前提：实验 3 建好的 srv/cli 拓扑还在
# ① 服务端脚本：accept 之后保持连接，不回数据也不关闭
cat > /tmp/lab_acc.py <<'PY'
import socket, sys, time
addr, port, backlog = sys.argv[1], int(sys.argv[2]), int(sys.argv[3])
s = socket.socket()
s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
...
回滚：命名空间删除时里面的进程会一并回收（见本节开头的统一清理）；确认 `ip netns exec srv ss -ti | grep 9998` 无输出即可。
...
> [!success] 核心结论已在实验机上实测（与 02~05 篇不同）
...
> - 复核补充：实验 1 的 **loopback 版本**（免建拓扑那条脚本）已在另一台 WSL2 Ubuntu / iproute2 6.1 上复跑，结论一致——`LISTEN 2 1 127.0.0.1:9999`、第 3 条起 `TimeoutError`、`TcpExtListenOverflows`/`ListenDrops` 同步增长 9、收尾后端口释放且无残留进程。
>
...
- [x] 速览卡 6 条，每条都是结论 + 可复现的证据（实测数字或计数命令）
- [x] 开篇有「读之前」的前置假设（实验机与权限、实验顺序、相邻笔记）与「分三遍读」路径，1.x 小节按【核心】/【进阶】分级
- [x] 有「名词速查」表，缩写与术语都能查到一句话解释与所在小节
- [x] 1.x 小节末尾标了对应实验；第 4 节开头给出「实验 ↔ 小节」对应表，并写明没有配套实验的小节与原因
- [x] 第 4 节的实验可自洽执行：需要后台运行的脚本都标了 `&`、引用了依赖的拓扑，且有统一清理命令
