...
| 类别 | 具体内容 | 备注 |
| --- | --- | --- |
| 内核控制结构 | 它复制 `task_struct`、调度实体、信号结构，以及 `task_struct` 里的统计字段 | 这部分开销不可省，它也是「创建风暴会消耗内核态 CPU」的原因 |
| 页表 | 它复制的是虚拟地址到物理页框的映射 | 这项开销与**映射的页数**成正比，因为看的是地址空间里到底映射了多少页（`VmSize` 大不等于页表多），所以它与常驻集（`VmRSS`）无关 |
| 资源与限制 | 它复制资源限制（`RLIMIT_*`）、信号处置与掩码、fd 表（引用同一批文件对象）、cwd、`umask`，以及进程组与会话 | 在运维上，「继承」这件事比「复制」更重要 |
...
**三，承诺额度在 `fork` 的瞬间就增加。** 提交计数（commit accounting）不因为「物理页是共享的」就免单。所以在严格超额承诺策略下（`vm.overcommit_memory=2`：额度上限是 `CommitLimit`，已承诺量是 `Committed_AS`），一个大的 `fork` 可能因为承诺额度不足而失败，即使物理内存看起来很充裕。

**`fork` 失败的两种错误码必须在第一时间分清**：

| 错误码 | 含义 | 典型来源 |
| --- | --- | --- |
| `EAGAIN` | 它表示资源暂时不可用 | 它出现在创建动作撞上某个任务数上限时：`RLIMIT_NPROC`（按 real UID 计）、cgroup 的 `pids.max`、`kernel.pid_max`、`kernel.threads-max` |
| `ENOMEM` | 它表示内存不足 | 它出现在内核结构或页表分配失败、承诺额度不足（`vm.overcommit_memory=2`），或子进程所在 PID namespace 的 init 已终止时 |

**`RLIMIT_AS`/`RLIMIT_DATA` 不约束 `fork`**——它们只在 `mmap`/`brk` 这类扩张地址空间的动作上生效，所以「地址空间被限住了就 fork 不出来」不是判据。

两者对应完全不同的修法，所以「起不了进程就把 `ulimit -u` 调大」是无效动作——它只可能命中四种上限中的一种，而且按 UID 计数的那一种**在持相关特权或 real UID 为 0 时根本不执行**，因此不能把它当作判据。
...
| 项目 | `execve` 之后 |
| --- | --- |
| PID / TGID | 它保持不变，因为被换掉的是程序而不是任务 |
| 地址空间、代码、堆、栈、映射 | 它们全部被替换 |
| 环境变量 | 它们由调用方传入（`execve(path, argv, envp)`） |
| 文件描述符 | 它们**默认保留**，除非标了 `FD_CLOEXEC`；0、1、2 是例外——若这三个 fd 在 exec 前是关闭的，而新程序带 set-user-ID/set-group-ID 位会让进程获得特权，系统可能替每个描述符打开一个未指定的文件，所以不能假设它们跨 `exec` 一定还是关着的 |
| 文件描述符的偏移量 | 偏移量保留，因为父子共用的还是同一个「打开文件描述」 |
| 信号处理函数 | 它们被**重置为默认动作**；被显式忽略的信号仍保持忽略 |
| 信号掩码 | 它保留 |
| 工作目录、`umask`、资源限制 | 它们都保留 |
| 进程组、会话、控制终端 | 它们都保留 |
| 父进程 | 它不变 |
| 已挂起的信号 | 它们保留——被屏蔽而挂起的信号不会因为 `exec` 消失，新程序仍能用 `sigpending`/`sigwait` 取到（多线程进程 exec 时，其余线程连同它们的线程级挂起信号一起被销毁） |
| 内存映射 | 它们全部消失，分三种情形：匿名 `mmap` 区被清除、附着的 System V 共享内存段被 detach（段本身还在，只是这个进程不再映射它）、POSIX 共享内存区被 unmap——这是「exec 之后共享内存段不见了」的原因 |
...
| 标志组 | 作用 | 得到什么 |
| --- | --- | --- |
| `CLONE_VM`、`CLONE_FS`、`CLONE_FILES`、`CLONE_SIGHAND`、`CLONE_THREAD`、`CLONE_SYSVSEM`、`CLONE_SETTLS`、`CLONE_PARENT_SETTID`、`CLONE_CHILD_CLEARTID` | 这组标志让新 task 共享地址空间、文件系统信息、fd 表、信号处置与线程组；其中后排四项负责 System V 信号量的 undo 语义、线程局部存储（TLS），以及 futex 要用的 TID 写回 | 它得到的是线程 |
| `CLONE_NEWPID`、`CLONE_NEWNET`、`CLONE_NEWNS`、`CLONE_NEWUSER`、`CLONE_NEWUTS`、`CLONE_NEWIPC`、`CLONE_NEWCGROUP` | 这组标志新建各类 namespace | 它得到的是容器里的进程视角 |
| `CLONE_PARENT`、`CLONE_PIDFD`、`CLONE_VFORK` | 这组标志让子进程与父进程同父、返回 pidfd，并让父进程阻塞直到子进程 exec 或退出 | 它用于特定用途 |
...
1. **第一段是复制内核控制结构**：它复制 `task_struct`、调度实体与信号结构；页表是另一笔开销，与映射的页数成正比；
2. **第二段是 `execve` 的加载**：它把可执行文件与依赖库映射进地址空间——这通常是「启动慢」的主要部分，也是「冷启动」与「热启动」差别的来源；
3. **第三段是调度与统计**：新 task 进运行队列，而短命 task 会带来大量上下文切换。
...
这条链条的关键在于**它是自我强化的**：CPU 时间被内核拿去创建进程，能用于干活的就少；进程被创建出来又立刻退出，PID 消耗加速；到了上限附近，重试逻辑往往转入高频重试，于是消耗被推得更高。所以 fork 风暴的处置方向是**在源头减量**（限流、改模型）；扩上限只是把触及上限的时间点往后推，而且会把整机上限的改动影响扩散到所有服务。
...
2. `setsid()`——它新建会话，从而脱离控制终端；
3. 再 `fork` 一次——这一步确保自己不是会话首进程，因此它永远无法再获得控制终端（这是第二步唯一没有覆盖的情形：会话首进程在打开一个尚未被占用的终端时可以获得它）；
4. 进程在这一步执行 `chdir("/")`、`umask(0)`，并把标准输入输出错误重定向到 `/dev/null` 或日志文件。
...
| 常见说法 | 事实 |
| --- | --- |
| 「`fork` 会复制整个内存，所以内存大的服务不能 fork」 | 物理页靠写时复制共享，因此成本落在页表与内核对象上，以及 `fork` 之后写入导致的复制上 |
| 「重启服务之后 PID 一定会变」 | 若进程自己 `exec` 自己来完成热重启，则 PID 不变、地址空间全换；只有「新 fork 再 exec」才换 PID |
| 「`exec` 之后一切都是新的」 | PID、fd、cwd、`umask`、资源限制与信号掩码全都保留，因此 fd 与信号掩码正是「重启后还占着旧资源」的原因 |
| 「`exec` 之后文件偏移量从头开始」 | 偏移量会保留，因为父子共用的还是同一个「打开文件描述」，所以这个偏移量是共享的 |
| 「子进程不会继承父进程打开的句柄」 | 子进程默认继承父进程打开的句柄，除非该 fd 标了 `FD_CLOEXEC`；这是句柄泄漏常见的形态之一——被这样占住的文件即使已经 `rm`，磁盘空间也不会还给文件系统 |
| 「`fork` 失败只有内存不足」 | `EAGAIN` 与 `ENOMEM` 是两类错误：前者来自撞上任务数上限，后者来自内核结构或页表分配失败与承诺额度不足，因此它们的判据与修法都不同 |
| 「高并发下每个请求 fork 一次也没问题，操作系统会处理」 | 每秒几万次创建会把 CPU 时间消耗在内核态，并把 PID 编号耗尽，因此这是自我强化的故障 |
| 「`vfork` 和 `fork` 差不多」 | 两者的差别是根本性的：`vfork` 的子进程在 `exec`/退出前直接共享父进程地址空间，可改动范围被压到只能写保存返回值的那一个 `pid_t`；Linux 还会把父进程挂起等它，但这一条是实现语义，POSIX 并未要求 |
| 「记住 PID 稍后再操作它是安全的」 | PID 会被复用，因此跨时间引用一个进程应该用 pidfd，而不是那个数字 |
| 「`nohup`/`setsid` 就是守护进程化」 | 它们只解决「别被终端带走」这一件事，而自启、重试、日志与资源上限都不在其中 |
...
答：它与映射的页数（页表规模）和内核对象数量相关，与常驻集大小不是线性关系。真正贵的情形是「fork 之后大量写」：每一页写入都要复制一次，峰值由写入量决定。

**问：`execve` 之后哪些东西保留？举两个因此产生的真实故障。**
答：它保留 PID、fd、cwd、`umask`、资源限制、信号掩码、进程组与会话。由此产生两个后果：默认继承的 fd 让旧连接/旧日志文件一直占着（`df` 与 `du` 对不上、端口仍被占用）；保留下来的信号掩码让新程序在无人处理的情况下继续屏蔽某个信号（表现为「对 `SIGTERM` 没反应」）。
...
答：风暴的根源是创建速率，因此扩上限只改变触及上限的时间点，并不改变速率；同时它会消耗更多 PID 与内核态 CPU，并让整机上限的改动波及所有服务。正确方向是在源头减量。
