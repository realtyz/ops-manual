...
> 内核自 2.6.25 起引入 `TASK_KILLABLE`：同样是不可中断睡眠，但**能被致命信号打断**（典型用于 NFS 的某些等待路径）。它是 `TASK_UNINTERRUPTIBLE | TASK_WAKEKILL` 的组合标志，**在 `/proc` 与 `ps` 里始终显示为 `D`**——内核的状态字母表里从来没有 `K`，别去搜「K 状态」。
...
> LOOP=$(losetup --find --show /tmp/lab-backing)         # 验证：拿到环设备名（**不要硬编码**，本机实测返回 /dev/loop0）
> lsblk -f "$LOOP"                                       # 验证：先确认这是本实验刚建的空设备，再执行下面的 mkfs
> dmsetup create lab-dstate --table "0 131072 linear $LOOP 0"   # 建一个 device-mapper 目标
...
> umount /mnt; dmsetup remove lab-dstate; losetup -d "$LOOP"; rm -f /tmp/lab-backing
