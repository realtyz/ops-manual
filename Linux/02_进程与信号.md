...
> 本笔记对应 [[Linux/00_简介|Linux 大纲]] 的「阶段 2：进程、信号与资源隔离」。目标：能说清进程的生死、状态与信号语义，能定位「卡死、杀不掉、起不来、悄悄消失」这四类问题，并且讲得清 namespace 与 cgroup 各管什么——这两样是后面所有容器问题的基础。
>
> 关联复习：[[01_启动流程与内核]]（PID 1 与 initramfs 里的进程）、[[03_内存管理与OOM]]（OOM Killer 是进程的第三种死法）、[[07_systemd与服务管理]]（服务的重启、限流与信号）、[[09_性能分析与排障]]（CPU 与上下文切换的分析方法）、[[Kubernetes/03_调度_资源_QoS|Kubernetes 资源与 QoS]]（cgroup 的编排层用法）。
>
...
| `numactl` | NUMA 绑核 + 绑内存 | — | 跨 NUMA 场景，详见 [[03_内存管理与OOM]] |
...
| `memory` | `memory.max`、`memory.high`、`memory.current`、`memory.stat`、`memory.events` | 内存上限、软上限、用量与 OOM 计数（详见 [[03_内存管理与OOM]]） |
...
> **namespace**（隔离视图）+ **cgroup**（限制资源）+ **rootfs/联合文件系统**（文件系统视图，见 [[04_存储与文件系统]]）+ **权限约束**（capabilities、seccomp、SELinux/AppArmor）。
...
| 整机范围的上限 | `/etc/sysctl.d/*.conf` | 见 [[01_启动流程与内核]] 的 sysctl 加载顺序 |
...
| 用 `top` 找高 CPU，找不到就认为没热点 | 线程级热点要看 `top -H`、`ps -eLf`、`pidstat -t`；单核打满时整机 CPU 可能只有 10% 左右（详见 [[09_性能分析与排障]]） |
...
| 用 `nohup`/`&` 起关键服务 | 无重试、无资源限制、无日志管理、无自启；写 unit 才是正解（见 [[07_systemd与服务管理]]） |
...
5. **最后才讨论放弃**：确认数据一致性风险后重启机器或触发 panic 取证（见 [[01_启动流程与内核]] 的 kdump）。
...
4. **是不是自己退的**：退出码 0 却被 systemd 判为失败，多半是 `Type=` 配错（如 `simple` 服务自己 daemonize），详见 [[07_systemd与服务管理]]。
...
> 单线程热点、绑核错误、GC 停世界都会表现为「整机看着空、延迟飙高」。这是面试里区分度很高的一道题（方法论见 [[09_性能分析与排障]]）。
...
- [x] 涉及时间的地方写明了时区与时间同步前提（`dmesg -T` 的陷阱沿用阶段 1 的结论）
- [x] 跨笔记引用都用 wikilink；跨目录引用用路径写法；指向尚未创建的阶段笔记（[[03_内存管理与OOM]]、[[04_存储与文件系统]]、[[07_systemd与服务管理]]、[[09_性能分析与排障]]）的链接与 [[Linux/00_简介|00_简介]] 的规划保持一致，目前在 Obsidian 中显示为未解析属预期
